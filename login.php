...
if ($ec_enabled) {
	...
	if (tep_session_is_registered ( 'paypal_ec_temp' ))
	...
	if (tep_session_is_registered ( 'paypal_ec_payer_id' ))

		tep_session_unregister ( 'paypal_ec_payer_id' );

	

	if (tep_session_is_registered ( 'paypal_ec_payer_info' ))
...
if ($error == true) {
	...
	$messageStack->add ( 'login', '<div>

' . TEXT_LOGIN_ERROR . '</div>' );
...
function session_win() {
  ...
  window.open("<?php
		...
		echo tep_href_link ( FILENAME_INFO_SHOPPING_CART );

		?>","info_shopping_cart","height=460,width=430,toolbar=no,statusbar=no,scrollbars=yes").focus();
...
<link href="/static/product_listing.css" rel="stylesheet"
...
<?php
 ...
 <div style="float:left;width:49%;"><label><?php echo ENTRY_EMAIL_ADDRESS; ?></label> <input type="text" name="email_address" class="inputbox" /></div>

 <div style="float:right;width:49%;"><label><?php echo ENTRY_PASSWORD; ?></label><?php echo tep_draw_password_field ( 'password' ); ?></div>
...
<li><?php echo ' <a href="' . tep_href_link ( FILENAME_PASSWORD_FORGOTTEN, '', 'SSL' ) . '">Forgot Passsword?</a>'; ?></li>
...
<h1>Or Checkout With Out a Account.</h1>

  Please note that all of our website account features will not be available to customers that do not wish to create an account.  
