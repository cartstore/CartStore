...
<?php
	...
	while ( list ( $wishlist_id, ) = each ( $wishList->wishID ) ) {
		...
		?>"><?php
		echo tep_image ( DIR_WS_IMAGES . $products ['products_image'], $products ['products_name'], 120, SMALL_IMAGE_HEIGHT );
		?></a>
...
<?php
	...
	echo '<button class="btn btn-default" name="delete_prod_x" value="true">Delete From Wishlist</button>';
	?>

     <?php
	echo '<button class="btn btn-default" name="add_prod_x" value="true">Add to Cart</button>';
	?></div>
				<div class="clear"></div>
...
<label><?php
	echo TEXT_MESSAGE . '</label> 
' . tep_draw_textarea_field2 ( 'message', 'soft', 45, 5 );
	?></div>
<?php
	echo tep_image_submit2 ( '', IMAGE_BUTTON_CONTINUE, 'name="email_prod"' );
	?>
	</form>
<?php
