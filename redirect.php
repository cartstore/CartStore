<?php
/*
  $Id: redirect.php,v 1.10 2003/06/05 23:31:31 hpdl & Farhad zare Exp $
  ...
  switch ($_GET['action']) {
    ...
    case 'url':
      ...
      break;
	      ...
	      case 'news':
      if (isset($_GET['goto']) && tep_not_null($_GET['goto'])) {
        $check_query = tep_db_query("select newsdesk_article_url from " . TABLE_NEWSDESK_DESCRIPTION . " where newsdesk_article_url = '" . tep_db_input($_GET['goto']) . "' limit 1");
        if (tep_db_num_rows($check_query)) {
          tep_redirect('http://' . $_GET['goto']);
        }
      }
