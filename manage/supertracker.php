...
/*
  ...
  Added keywords filters by Monika Math�, http://www.monikamathe.com
*/
...
// ********** PAY PER CLICK CONFIGURATION SECTION EOF ************
  ...
  function draw_geo_graph($geo_hits,$country_names,$total_hits) {
	...
	if ($action == 'del_rows' && isset($_POST['num_rows'])) {
    $rows_to_delete = (int)$_POST['num_rows'];
...
<div class="form-group"><h3><?php echo TEXT_DATABASE_INFO; ?></h3>
<?php
						...
						echo '<p>' . sprintf(TEXT_TABLE_DATABASE, $num_rows, tep_date_short($maint_row['time_arrived'])) . '</p>';
						echo '<form name="del_rows" action="supertracker.php?action=del_rows" method="post"><p>' . TEXT_TABLE_DELETE . '</p>
						<input class="form-control" name="num_rows" size=10></div><p><input type="submit" class="btn btn-default" value="' . TEXT_BUTTON_ERASE  . '"></p></form>';
?>


		<div class="form-group"><label> <?php echo TABLE_TEXT_MENU_DESC_TEXT; ?></label> 
						 
						 <form name="report_select"><select name="report_selector"  class="form-control" onChange="page_redirect(this)">
				  ...
				  $product_id = rtrim($product_id, '?');
					if ($product_id != '') {
					  ...
					  if (tep_not_null($prod_row['products_image'])) {
  					    echo '<td><table cellspacing=0 cellpadding=2 border=0 align="center"><tr><td align="center">' . tep_image(DIR_WS_CATALOG_IMAGES . $prod_row['products_image'], $prod_row['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</td></tr><tr><td class="dataTableContent">' . $prod_row['products_name'] . '</td></tr></table></td>';
                      } else {
  					    echo '<td><table cellspacing=0 cellpadding=2 border=0 align="center"><tr><td align="center">' . tep_image(DIR_WS_CATALOG_IMAGES . 'st_no_image.jpg', $prod_row['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</td></tr><tr><td class="dataTableContent">' . $prod_row['products_name'] . '</td></tr></table></td>';
					  }
					}				
				}
  			  echo '</tr></table></td></tr>';
...
?>
<br><strong><a href="supertracker.php?special=last_ten&offset=<?php echo $offset + 10;?>&filter=<?php echo $filter;?>&refer_match=<?php echo $refer_match;?>"><input type="submit" class="btn btn-default" value="<?php echo TABLE_TEXT_NEXT_TEN_RESULTS; ?>"></a></strong>
<?php
