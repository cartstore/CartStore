...
?>
                <td class="dataTableContent"><?php if ( (is_object($oInfo)) && ($orders_status['refund_method_id'] == $oInfo->refund_method_id) ) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_REFUND_METHODS, 'page=' . $_GET['page'] . '&oID=' . $orders_status['refund_method_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
...
?>
                  <tr>
                    <td colspan="2" align="right"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_REFUND_METHODS, 'page=' . $_GET['page'] . '&action=new') . '">' .  IMAGE_INSERT . '</a>'; ?></td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_REFUND_METHODS, 'page=' . $_GET['page']) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_REFUND_METHODS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->refund_method_id) . '">' . IMAGE_CANCEL . '</a>');
      break;
    case 'delete':
      ...
      if ($remove_status) $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_REFUND_METHODS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->refund_method_id) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    default:
      if (is_object($oInfo)) {
        $heading[] = array('text' => '<b>' . $oInfo->refund_method_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_REFUND_METHODS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->refund_method_id . '&action=edit') . '">' .  IMAGE_EDIT . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_REFUND_METHODS, 'page=' . $_GET['page'] . '&oID=' . $oInfo->refund_method_id . '&action=delete') . '">' .  IMAGE_DELETE . '</a>');
