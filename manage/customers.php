...
On this page you can edit the customer account details.                        </div>
                      </div>
                  </div>   
              </div>    
...
<table class="table">
	...
	echo '<h3>'. HEADING_TITLE_MODULES_PAYMENT .'</h3>'; ?></td>
      </tr>
      <tr>
        <td class="formArea"><table class="table">
          <tr class="success">
            <td class="success" colspan="2"><?php if ($processed == true) {
...
<?php
	...
	?>
	   <tr>
            <td class="main" colspan="2"><div class="alert alert-info"><?php echo ENTRY_CUSTOMERS_PAYMENT_SET_EXPLAIN ?></div></td>
           </tr>
<?php
...
<h3><?php echo HEADING_TITLE_MODULES_SHIPPING; ?></h3>
...
<?php
	...
	?>
	   <tr>
            <td class="main"><div class="alert alert-info"><?php echo ENTRY_CUSTOMERS_SHIPPING_SET_EXPLAIN ?></div></td>
           </tr>
<?php
   } else { // end if ($processed != true)
?>
...
<p>
<?php echo tep_image_submit('button_update.png', IMAGE_UPDATE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('action'))) .'">' .  IMAGE_CANCEL . '</a>'; ?>
...
<div class="page-header"><h1><a class="pull-right" data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
            ...
            <div class="panel-group" id="accordion">
                  ...
                  <div class="panel panel-default">
                      ...
                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body"><i class="fa fa-users fa-5x pull-left"></i>
 Customer management section, here you can edit customer accounts, create orders and email them.                          </div>
                      </div>
                  </div>   
              </div>    
  
<div class="form-group"><label><?php echo HEADING_TITLE_SEARCH . '</label> ' . tep_draw_input_field('search'); ?></div>
          ...
          ?>
             <td valign="top"><table class="table table-hover table-condensed table-responsive">
              <tr class="dataTableHeadingRow">
                <td class="dataTableHeadingContent" valign="top"><a href="<?php echo "$PHP_SELF?listing=company"; ?>"><i class="fa fa-chevron-up"></i></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=company-desc"; ?>"><i class="fa fa-chevron-down"></i></a><br><?php echo ENTRY_COMPANY; ?></td>
                <td class="dataTableHeadingContent" valign="top"><a href="<?php echo "$PHP_SELF?listing=lastname"; ?>"><i class="fa fa-chevron-up"></i></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=lastname-desc"; ?>"><i class="fa fa-chevron-down"></i></a><br><?php echo TABLE_HEADING_LASTNAME; ?></td>
                <td class="dataTableHeadingContent" valign="top"><a href="<?php echo "$PHP_SELF?listing=firstname"; ?>"><i class="fa fa-chevron-up"></i></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=firstname-desc"; ?>"><i class="fa fa-chevron-down"></i></a><br><?php echo TABLE_HEADING_FIRSTNAME; ?></td>
		<td class="dataTableHeadingContent" valign="top"><a href="<?php echo "$PHP_SELF?listing=cg_name"; ?>"><i class="fa fa-chevron-up"></i></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=cg_name-desc"; ?>"><i class="fa fa-chevron-down"></i></a><br><?php echo TABLE_HEADING_CUSTOMERS_GROUPS; ?></td>
                <td class="dataTableHeadingContent" valign="top"><a href="<?php echo "$PHP_SELF?listing=id-asc"; ?>"><i class="fa fa-chevron-up"></i></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=id-desc"; ?>"><i class="fa fa-chevron-down"></i></a><br><?php echo TABLE_HEADING_ACCOUNT_CREATED; ?>
                                </td><td class="dataTableHeadingContent" align="middle" valign="top"><a href="<?php echo "$PHP_SELF?listing=ra"; ?>"><i class="fa fa-chevron-up"></i></a>&nbsp;<a href="<?php echo "$PHP_SELF?listing=ra-desc"; ?>"><i class="fa fa-chevron-down"></i></a><br><?php echo TABLE_HEADING_REQUEST_AUTHENTICATION; ?>&nbsp;</td>
                </td><td class="dataTableHeadingContent" valign="top"><br><?php echo TABLE_HEADING_ACTION; ?>&nbsp;</td>
              </tr>
...
// EOF customer_sort_admin_v1 adapted for Separate Pricing Per Customer ?></td>
                <td class="dataTableContent"><?php echo tep_date_short($info['date_account_created']); ?></td>
...
<?php
      if ($customers['customers_group_ra'] == '1') {
        ...
        echo '';
                ...
                <td class="dataTableContent"><?php if (isset($cInfo) && is_object($cInfo) && ($customers['customers_id'] == $cInfo->customers_id)) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
?>
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS) . '">' . IMAGE_RESET . '</a>'; ?></td>
                  </tr>
<?php
    }
?>
   ...
   for ($z = 0; $z < sizeof($count_groups_array); $z++) {
	  $bgcolor = ($c++ & 1) ? ' class="dataTableRow"' : '';
	   ?>
		 ...
		 <td class="dataTableContent"><?php echo $count_groups_array[$z]['number_in_group'] ?></td>
		 </tr>
<?php
...
// BOF Separate Pricing Per Customer: dark grey field with customer name higher
      $heading[] = array('text' => '<br><b>' . TEXT_INFO_HEADING_DELETE_CUSTOMER . '</b>');
// EOF Separate Pricing Per Customer
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id) . '">' . IMAGE_CANCEL . '</a>');
      break;
    default:
      if (isset($cInfo) && is_object($cInfo)) {
// BOF Separate Pricing Per Customer: dark grey field with customer name higher
        $heading[] = array('text' => '<br><b>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</b>');
// EOF Separate Pricing Per Customer


        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . IMAGE_EDIT . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '">' .  IMAGE_DELETE . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '">' .  IMAGE_ORDERS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . IMAGE_EMAIL . '</a>');

        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link('../password_forgotten.php', 'cID=' . $cInfo->customers_id) . '" target="_BLANK">Send new password</a><br><a class="btn btn-default" href="' . tep_href_link('create_order.php', 'Customer=' . $cInfo->customers_id) . '">Create Order</a>');
...
$customersrecords = tep_db_query("SELECT * FROM customers") or die ("What Happen??? Error 1");
while($customerrows = tep_db_fetch_array($customersrecords))
{
$e = tep_db_query("SELECT * FROM address_book WHERE customers_id ='$customerrows[customers_id]'") or die ("What Happen??? Error
2");
$real = tep_db_fetch_array($e);
$updatedefaultaddress = tep_db_query("UPDATE customers SET customers_default_address_id = '$real[address_book_id]' WHERE
