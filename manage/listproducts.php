...
<?php
   ...
   $get_result=tep_db_fetch_array($coupon_get);
    ...
    for ($i = 0; $i < count($pr_ids); $i++) {
      $result =tep_db_query("SELECT * FROM " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd WHERE p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "'and p.products_id = '" . $pr_ids[$i] . "'");
