              ...
              <div class="panel-group" id="accordion">
                  ...
                  <div class="panel panel-default">
                      ...
                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body"><i class="fa fa-truck fa-5x pull-left"></i>
...
<?php echo TEXT_BOX_ADMIN; ?> <table border="0" cellspacing="2" cellpadding="2">
        <tr>
...
<tr>
                ...
                <td class="main"> <?php echo  tep_draw_pull_down_menu('vendors_status_send', $orders_statuses, $vInfo->vendors_status_send); ?></td>
              </tr>
</td>
          </tr>
           <tr>
...
<?php
 if ($vInfo->vendors_send_email == '0') {
    ...
    echo tep_draw_radio_field('vendors_send_email', '1', $in_status) . '&nbsp;' . TEXT_SEND_ORDER_EMAIL_YES . '&nbsp;' . tep_draw_radio_field('vendors_send_email', '0', $out_status) . '&nbsp;' . TEXT_SEND_ORDER_EMAIL_NO;
?></td>
          </tr>
          <tr>
...
?>
          ...
          <tr>
            <td class="main" align="top"><?php echo TEXT_VENDORS_IMAGE; ?>
                    <td class="main"><?php echo tep_draw_file_field('vendors_image')  . ' ' . $vInfo->vendors_image . tep_draw_hidden_field('previous_vendors_image', $vInfo->vendors_image);
     //  echo tep_draw_hidden_field('vendors_image', stripslashes($vendors_image_name));

       echo tep_draw_checkbox_field('delete_vendors_image', 'yes', false) . TEXT_DELETE_IMAGE;
      ...
      <tr>
        <td align="right" class="main">
                ...
                <?php
                        if($action == 'new') { echo tep_image_submit('button_insert.png', IMAGE_INSERT) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('action'))) .'">' .  IMAGE_CANCEL . '</a>'; }
                else { ?>

                <?php echo tep_image_submit('button_update.png', IMAGE_UPDATE, 'action=update') . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('action'))) .'">' .  IMAGE_CANCEL . '</a>'; ?>
...
<?php
      if ($vendor_data['vendors_send_email'] == '1') {
        echo '<a href="' . tep_href_link(FILENAME_VENDORS, 'action=setflag&flag=0&vendors_id=' . $vendor_data['vendors_id']) . '"> <i class="fa fa-times-circle-o text-danger"></i></a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_VENDORS, 'action=setflag&flag=1&vendors_id=' . $vendor_data['vendors_id']) . '"> <i class="fa fa-check-circle-o text-success"></i></a>';
      }
      ?></td>
                <td class="dataTableContent"><?php if (isset($vInfo) && is_object($vInfo) && ($vendor_data['vendors_id'] == $vInfo->vendors_id)) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('vendors_id')) . 'vendors_id=' . $vendor_data['vendors_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
?>
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDORS) . '">' .  IMAGE_RESET . '</a>'; ?></td>
                  </tr>
<?php
    }
?>
...
<?php
  ...
  switch ($action) {
     ...
     while ($count_products = tep_db_fetch_array($count_products_query)) {
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('vendors_id', 'action')) . 'vendors_id=' . $vInfo->vendors_id) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    default:
   ...
   $num_orders = 'no';
        ...
        $heading[] = array('text' => '<b>' . $vInfo->vendors_name . ' ' . $vInfo->vendors_contact . '</b>');
                ...
                <a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('vendors_id', 'action')) . 'vendors_id=' . $vInfo->vendors_id . '&action=edit') . '">' .  IMAGE_EDIT . '</a> 
                ' . ($vInfo->vendors_id == DEFAULT_VENDOR_ID ? '' : '<a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('vendors_id', 'action')) . 'vendors_id=' . $vInfo->vendors_id . '&action=confirm') . '">' . IMAGE_DELETE . '</a>') . 
                '<br><a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDOR_MODULES, 'set=shipping&vendors_id=' . $vInfo->vendors_id) . '">' .  IMAGE_MANAGE . '</a>
                &nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDORS, tep_get_all_get_params(array('vendors_id', 'action')) . 'action=new') . '">' . IMAGE_INSERT . '</a>
                <br><a class="btn btn-default" href="' . tep_href_link(FILENAME_ORDERS_VENDORS, '&vendors_id=' . $vInfo->vendors_id) . '">' .  IMAGE_ORDERS . '</a><br><br>There are <strong>' . $num_orders . '</strong> orders for this Vendor.<br>This Vendor has <strong>' . $num_products . '</strong> products.<br>Click <a href="' . tep_href_link(FILENAME_PRODS_VENDORS, '&vendors_id=' . $vInfo->vendors_id) . '"><strong>here </strong></a>for Products Report<br>Click <a href="' . tep_href_link(FILENAME_STATS_VENDORS, '&vID=' . $vInfo->vendors_id) . '"><strong>here </strong></a>for Vendor Sales Report<strong> (Coming Soon)</strong>');
