...
You are editing/creating a article category. Choose its name, seo meta title and description which also services as the section/ categories meta description tag.                        </div>
                      </div>
                  </div>   
              </div> 
...
<?php
		...
		for($i = 0; $i < sizeof ( $languages ); $i ++) {
          ...
          <tr>
			...
			?>&nbsp;<span class="class="mceEditor"><p><?php
				echo tep_draw_textarea_field_redactor('topics_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($topics_description[$languages[$i]['id']]) ? stripslashes($topics_description[$languages[$i]['id']]) .'</p>' : tep_get_topic_description($tInfo -> topics_id, $languages[$i]['id'])));
				?></span>
						</td>
...
<?php
		...
		?></td>
						...
						<td class="main"><?php
						echo  tep_draw_input_field('sort_order', $tInfo -> sort_order, 'size="2"');
		?></td>
				...
				<p><?php
					echo tep_draw_hidden_field('topics_date_added', (($tInfo -> date_added) ? $tInfo -> date_added : date('Y-m-d'))) . tep_draw_hidden_field('parent_id', $tInfo -> parent_id) . (isset ( $_GET ['tID'] ) ? tep_image_submit('button_update.png', IMAGE_UPDATE) : tep_image_submit('button_insert.png', IMAGE_INSERT)) . '&nbsp;&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $_GET['tID']) . '">' . IMAGE_CANCEL . '</a>';
				?></p>
...
<p>
    <?php
				echo '<a class="btn btn-default" href="' . tep_href_link($back_url, $back_url_params, 'NONSSL') . '">' . IMAGE_BACK . '</a>';
			?> 
</p>
<?php
...
echo '&nbsp;&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $_GET['tID']) . '">' . IMAGE_CANCEL . '</a>';
			?> 
</p>
				</form>
...
<div class="page-header"><h1><a class="pull-right" data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
   ...
   <?php
		...
		?>
		</h1></div>          <div class="panel-group" id="accordion">
                  <div class="clear"></div>
                  <div class="panel panel-default">

                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body"><i class="fa fa-question-circle fa-5x pull-left"></i>
qqqqHelp for this section is not yet available.                          </div>
                      </div>
                  </div>   
              </div> 
		...
		?></td>
							...
							<td class="main"><?php
							echo tep_draw_radio_field('articles_status', '0', $out_status) . '&nbsp;' . TEXT_ARTICLE_NOT_AVAILABLE . '&nbsp;' . tep_draw_radio_field('articles_status', '1', $in_status) . '&nbsp;' . TEXT_ARTICLE_AVAILABLE;
		?></td>
		...
		for($i = 0, $n = sizeof ( $languages ); $i < $n; $i ++) {
          ...
          <tr>
			...
			?><?php
				...
				echo tep_draw_textarea_field_redactor('articles_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (isset($articles_description[$languages[$i]['id']]) ? $articles_description[$languages[$i]['id']] : tep_get_articles_description($aInfo -> articles_id, $languages[$i]['id'])));
				?></td>
...
<p>
       <?php
					echo tep_draw_hidden_field('articles_date_added', (tep_not_null($aInfo -> articles_date_added) ? $aInfo -> articles_date_added : date('Y-m-d'))) . (isset ( $_GET ['aID'] ) ? tep_image_submit('button_update.png', IMAGE_UPDATE) : tep_image_submit('button_insert.png', IMAGE_INSERT)) . '&nbsp;&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . (isset($_GET['aID']) ? '&aID=' . $_GET['aID'] : '')) . '">' . IMAGE_CANCEL . '</a>';
		?></p>
...
echo ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . (isset($_GET['aID']) ? '&aID=' . $_GET['aID'] : '')) . '">' . IMAGE_CANCEL . '</a>';
			?> 
			</form>
		...
		?></h1></div>
          <div class="panel-group" id="accordion">
                  ...
                  <div class="panel panel-default">
                      ...
                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body"><i class="fa fa-question-circle fa-5x pull-left"></i>
This is the general article section. We use these for store informational pages, like about us, terms etc. These pages can be assessed either through menu links in the template or by the articles module.                         </div>
                      </div>
                  </div>   
              </div>    
...
echo '<div class="form-group hide"><label>'. HEADING_TITLE_SEARCH . '</label>' . tep_draw_input_field('search');
...
echo '<div class="form-group"><label>'. HEADING_TITLE_GOTO . ' </label>' . tep_draw_pull_down_menu('tPath', tep_get_topic_tree(), $current_topic_id, 'onChange="this.form.submit();"');
...
<?php
		...
		while ( $topics = tep_db_fetch_array ( $topics_query ) ) {
			...
			?>
				...
				echo '<a href="' . tep_href_link(FILENAME_ARTICLES, tep_get_topic_path($topics['topics_id'])) . '"><i class="fa fa-eye text-info"></i> </a>&nbsp;<b>' . $topics['topics_name'] . '</b>';
			?></td>

								<td class="dataTableContent"><?php
								echo '<a href="' . HTTP_SERVER . '' . DIR_WS_CATALOG . 'articles.php?tPath=' . $topics['topics_id'] . '" target="_BLANK"><i class="fa fa-link text-warning"></i></a>';
			?>
								...
								if (isset($tInfo) && is_object($tInfo) && ($topics['topics_id'] == $tInfo -> topics_id)) {
									...
									echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $topics['topics_id']) . '"><i class="fa fa-hand-o-up"></i></a>';
								}
			?>&nbsp;</td>
...
echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link ( FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $articles ['articles_id'] ) . '\'">' . "\n";
			...
			?>
                ...
                <td class="dataTableContent"><?php
				echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $articles['articles_id'] . '&action=article_preview&read=only') . '"><i class="fa fa-eye text-info"></i> </a> ' . $articles['articles_name'];
			?></td>

								<td class="dataTableContent"><?php
								echo '<a href="' . HTTP_SERVER . '' . DIR_WS_CATALOG . 'article_info.php?articles_id=' . $articles['articles_id'] . '" target="_BLANK"><i class="fa fa-link text-warning"></i></a>';
			?>
...
if ($articles['articles_status'] == '1') {
	...
	echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'action=setflag&flag=0&aID=' . $articles['articles_id'] . '&tPath=' . $tPath) . '"><i class="fa fa-check-circle-o text-success"></i> </a>';

} else {

	echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'action=setflag&flag=1&aID=' . $articles['articles_id'] . '&tPath=' . $tPath) . '"><i class="fa fa-times-circle-o text-danger"></i> </a>';
			...
			?></td>
								...
								if (isset($aInfo) && is_object($aInfo) && ($articles['articles_id'] == $aInfo -> articles_id)) {
									...
									echo '<a href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $articles['articles_id']) . '"><i class="fa fa-hand-o-up"></i></a>';
								}
			?>&nbsp;</td>
...
$tPath_back = (tep_not_null ( $tPath_back )) ? 'tPath=' . $tPath_back . '&' : '';
		...
		?></td>
											...
											if (sizeof($tPath_array) > 0)
												echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, $tPath_back . 'tID=' . $current_topic_id) . '">' . IMAGE_BACK . '</a>&nbsp;';
											if (!isset($_GET['search']))
												echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&action=new_topic') . '">' . IMAGE_NEW_TOPIC . '</a>&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&action=new_article') . '">' . IMAGE_NEW_ARTICLE . '</a>';
		?>&nbsp;</td>
...
switch ($action) {
	...
	case 'new_topic' :
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.png', IMAGE_SAVE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath) . '">' . IMAGE_CANCEL . '</a>');
	...
	case 'edit_topic' :
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.png', IMAGE_SAVE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo -> topics_id) . '">' . IMAGE_CANCEL . '</a>');
	...
	case 'delete_topic' :
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo -> topics_id) . '">' . IMAGE_CANCEL . '</a>');
	...
	case 'move_topic' :
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_move.png', IMAGE_MOVE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo -> topics_id) . '">' . IMAGE_CANCEL . '</a>');
	...
	case 'delete_article' :
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo -> articles_id) . '">' . IMAGE_CANCEL . '</a>');
	...
	case 'move_article' :
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_move.png', IMAGE_MOVE) . ' <a class="btn btn-default"  href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo -> articles_id) . '">' . IMAGE_CANCEL . '</a>');
	...
	case 'copy_to' :
		...
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_copy.png', IMAGE_COPY) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo -> articles_id) . '">' . IMAGE_CANCEL . '</a>');
	...
	default :
		if ($rows > 0) {
			...
			if (isset($tInfo) && is_object($tInfo)) {// topic info box contents
				...
				$contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo -> topics_id . '&action=edit_topic') . '">' . IMAGE_EDIT . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo -> topics_id . '&action=delete_topic') . '">' . IMAGE_DELETE . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $tInfo -> topics_id . '&action=move_topic') . '">' . IMAGE_MOVE . '</a>');
				...
				$contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo -> articles_id . '&action=new_article') . '">' . IMAGE_EDIT . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo -> articles_id . '&action=delete_article') . '">' . IMAGE_DELETE . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo -> articles_id . '&action=move_article') . '">' . IMAGE_MOVE . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_ARTICLES, 'tPath=' . $tPath . '&aID=' . $aInfo -> articles_id . '&action=copy_to') . '">' . IMAGE_COPY_TO . '</a>');
