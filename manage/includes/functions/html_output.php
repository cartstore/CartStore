<?php
  ...
  {
      $image = '<img class="thumbnail" src="' . tep_output_string($src) . '" border="0" alt="' . tep_output_string($alt) . '"';
      if (tep_not_null($alt)) {
  ...
  {
      global $language;
      $image_submit = '<input type="submit" class="btn btn-default" value="' . tep_output_string($alt) . '"';
      if (tep_not_null($alt))
          $image_submit .= '';
      if (tep_not_null($parameters))
  ...
  {
      $field = '<input class="btn btn-defualt" type="' . tep_output_string($type) . '" name="' . tep_output_string($name) . '"';
      if (isset($GLOBALS[$name]) && ($reinsert_value == true) && is_string($GLOBALS[$name])) {
          $field .= ' value="' . tep_output_string(stripslashes($GLOBALS[$name])) . '"';
      } elseif (tep_not_null($value)) {
          $field .= ' value="' . tep_output_string($value) . '"';
  ...
  {
      $field = '<input class="form-control" type="text" name="' . $name . '" size="' . $size . '" maxlength="' . $numchar . '" value="';
      if ($params)
          $field .= '' . $params;
      $field .= '';
  ...
  function tep_draw_textarea_field_redactor($name, $wrap, $width, $height, $text = '', $parameters = '', $reinsert_value = true)
  {
      $field = '<textarea class="redactor"  name="' . tep_output_string($name) . '" wrap="' . tep_output_string($wrap) . '" cols="' . tep_output_string($width) . '" rows="' . tep_output_string($height) . '"';
      if (tep_not_null($parameters))
          $field .= ' ' . $parameters;
      $field .= '>';
      if ((isset($GLOBALS[$name])) && ($reinsert_value == true)) {
