...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
 ...
 <table  class="table table-hover">
              <tr class="dataTableHeadingRow">
                ...
                <TD class="dataTableHeadingContent" width=10%> <?php echo TABLE_HEADING_ACTION; ?></TD>
              </tr>
<?php
  $page = $_GET['page'];
  if (!$page) $page = 1;
...
?>
                ...
                <td class="dataTableContent"><?php if ($products_extra_image['products_extra_images_id'] == $pId) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_PRODUCTS_EXTRA_IMAGES, 'page=' . $page . '&pID=' . $pId) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>				
              </tr>
<?php
  }
?>
...
?>
              <tr>
                <td align="right" colspan="3" class="smallText"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_PRODUCTS_EXTRA_IMAGES, 'page=' . $page . '&pId=' . $pInfo->products_extra_images_id . '&action=new') . '">' .  IMAGE_INSERT . '</a>'; ?></td>
              </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.png', IMAGE_SAVE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_PRODUCTS_EXTRA_IMAGES, 'page=' . $page . '&pId=' . $pId) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    case 'edit':
      ...
      $contents[] = array('text' => '<br>' . TEXT_PRODUCTS . ' ' . (count($products_array)-1));
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.png', IMAGE_SAVE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_PRODUCTS_EXTRA_IMAGES, 'page=' . $page . '&pId=' . $pInfo->products_extra_images_id) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_PRODUCTS_EXTRA_IMAGES, 'page=' . $page . '&pId=' . $pInfo->products_extra_images_id) . '">' .  IMAGE_CANCEL . '</a>');
      break; 
    default:
      if (isset($pId)) {
        $heading[] = array('text' => '<b>' . $pInfo -> products_model . '</b>');
        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_PRODUCTS_EXTRA_IMAGES, 'page=' . $page . '&pId=' . $pId . '&action=edit') . '">' . IMAGE_EDIT . '</a><a class="btn btn-default" href="' . tep_href_link(FILENAME_PRODUCTS_EXTRA_IMAGES, 'page=' . $page . '&pId=' . $pId . '&action=delete') . '">' .  IMAGE_DELETE . '</a>');
