...
<p>Note: While the newsletter supports html it does not at this time support images unless you put in the full url to the images on your server.
</p>
...
<p>
<?php echo (($form_action == 'insert') ? tep_image_submit('button_save.png', IMAGE_SAVE) : tep_image_submit('button_update.png', IMAGE_UPDATE)). '&nbsp;&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . (isset($_GET['nID']) ? 'nID=' . $_GET['nID'] : '')) . '">' . IMAGE_CANCEL . '</a>'; ?>
...
<?php echo nl2br($nInfo->content); ?>
<p>
<?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . IMAGE_BACK . '</a>'; ?>
</p>
...
<?php echo tep_image(DIR_WS_IMAGES . 'ani_send_email.png', IMAGE_ANI_SEND_EMAIL); ?>
...
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $newsletters['newsletters_id'] . '&action=preview') . '"></a>' . $newsletters['title']; ?></td>
                <td class="dataTableContent"><?php echo number_format($newsletters['content_length']) . ' bytes'; ?></td>
                <td class="dataTableContent"><?php echo $newsletters['module']; ?></td>
                <td class="dataTableContent"><?php if ($newsletters['status'] == '1') { echo '<i class="fa fa-check-circle-o text-success"></i>'; } else { echo '<i class="fa fa-times-circle-o text-danger"></i>'; } ?></td>
                <td class="dataTableContent"><?php if ($newsletters['locked'] > 0) { echo '<i class="fa fa-lock text-success"></i>'; } else { echo '<i class="fa fa-unlock-alt text-danger"></i>'; } ?></td>
                <td class="dataTableContent"><?php if (isset($nInfo) && is_object($nInfo) && ($newsletters['newsletters_id'] == $nInfo->newsletters_id) ) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $newsletters['newsletters_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
              <tr>
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  ...
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'action=new') . '">' .  IMAGE_NEW_NEWSLETTER . '</a>'; ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    default:
      if (is_object($nInfo)) {
        $heading[] = array('text' => '<b>' . $nInfo->title . '</b>');
        ...
        if ($nInfo->locked > 0) {
          $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->newsletters_id . '&action=new') . '">' . IMAGE_EDIT . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->newsletters_id . '&action=delete') . '">' .  IMAGE_DELETE . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->newsletters_id . '&action=preview') . '">' .  IMAGE_PREVIEW . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->newsletters_id . '&action=send') . '">' . IMAGE_SEND . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->newsletters_id . '&action=unlock') . '">' .  IMAGE_UNLOCK . '</a>');
        } else {
          $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->newsletters_id . '&action=preview') . '">' .  IMAGE_PREVIEW . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $nInfo->newsletters_id . '&action=lock') . '">' .  IMAGE_LOCK . '</a>');
        }
