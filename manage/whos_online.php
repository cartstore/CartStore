...
// Images used for status lights
  $status_active_cart = '<i class="fa fa-check-circle-o  text-success"></i> ';
  $status_inactive_cart = '<i class="fa fa-times-circle-o text-success"></i> ';
  $status_active_nocart = '<i class="fa fa-check-circle-o text-warning"></i> ';
  $status_inactive_nocart = '<i class="fa fa-times-circle-o text-warning"></i> ';
  $status_active_bot = '<i class="fa fa-check-circle-o text-danger"></i> ';
  $status_inactive_bot = '<i class="fa fa-times-circle-o text-danger"></i> ';
...
function tep_check_cart($which, $customer_id, $session_id) {
  ...
  if( $customer_id < 0 ) {
    ...
    if ($who_query['time_last_click'] < $xx_mins_ago_long) {
      return '<label>'. $status_inactive_bot .'' . TEXT_STATUS_INACTIVE_BOT .'</label>';
	// active
    } else {
      return '<label>'.  $status_active_bot . '' . TEXT_STATUS_ACTIVE_BOT.'</label>';
    }
	}
  ...
  if ( sizeof($products) == 0 ) {
    // inactive
    if ($who_query['time_last_click'] < $xx_mins_ago_long) {
      return '<label>'.  $status_inactive_nocart .''. TEXT_STATUS_INACTIVE_NOCART .'</label>';
    // active
    } else {
      return '<label>'.  $status_active_nocart .''. TEXT_STATUS_ACTIVE_NOCART .'</label>';
    }
  // cart
	} else {
    // inactive
    if ($who_query['time_last_click'] < $xx_mins_ago_long) {
      return '<label>'.  $status_inactive_cart .''.  TEXT_STATUS_INACTIVE_CART .'</label>';
    // active
    } else {
      return '<label>'.  $status_active_cart .''.  TEXT_STATUS_ACTIVE_CART .'</label>';
    }
  }
}
            ...
            <!-- Status Legend - Uses variables for image names -->
              ...
              <tr>
                <td class="smallText"><?php echo
                 '<label>'. $status_active_cart . ' ' . TEXT_STATUS_ACTIVE_CART.'</label>';
              ?></td>
           	    <td class="smallText"><?php echo
               '<label>'.  $status_inactive_cart. ' ' . TEXT_STATUS_INACTIVE_CART.'</label>';
              ?></td>
              </tr>
              <tr>
                <td class="smallText"><?php echo
                  '<label>'. $status_active_nocart . ' ' . TEXT_STATUS_ACTIVE_NOCART.'</label>';
              ?></td>
           	    <td class="smallText"><?php echo
                  '<label>'. $status_inactive_nocart . ' ' . TEXT_STATUS_INACTIVE_NOCART.'</label>';
              ?></td>
              </tr>
              <tr>
                <td class="smallText"><?php echo
                  '<label>'. $status_active_bot . ' ' . TEXT_STATUS_ACTIVE_BOT.'</label>';
              ?></td>
           	    <td class="smallText"><?php echo
                  '<label>'. $status_inactive_bot . ' ' . TEXT_STATUS_INACTIVE_BOT.'</label>';
             ?></td>
             </tr>
             </table>
           </td>
          ...
          <tr>
            <td valign="top"><table class="table table-hover table-condensed table-responsive">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_USER_SESSION; ?>&nbsp;</td>
