  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'update':
        ...
        $banners_sort_order = tep_db_prepare_input($_POST['banners_sort_order']);
        $db_image_location = '';
...
// Build a language array for the pulldown
  ...
  foreach( $language_array as $language_info ) {
    $languages_data[] = array(
      'id' => $language_info['id'],
...
Here you can create banners both html and text. The groups field controls where it shows on the template. This positioning is defined when your site is created. If you need additional placement for extra banner groups please contact cartstore so that can be added to your template.                         </div>
                      </div>
                  </div>   
              </div>    
...
// Banner Rotator;
      $banner_query = tep_db_query("select language_id, banners_title, banners_url, banners_image, banners_group, banners_html_text, status, sort_order, date_format(date_scheduled, '%d/%m/%Y') as date_scheduled, date_format(expires_date, '%d/%m/%Y') as expires_date, expires_impressions, date_status_change from " . TABLE_BANNERS . " where banners_id = '" . (int)$bID . "'");
      $banner = tep_db_fetch_array($banner_query);
...
<?php // Start Banner Rotator; ?>
          <tr>
            <td class="main"></td>
            <td class="main"><?php echo tep_draw_pull_down_menu( 'language_id', $languages_data, $bInfo->language_id); ?></td>
          </tr>
<?php // End Banner Rotator; ?>
          ...
          <tr>
            <td valign="top"   class="main"><?php echo TEXT_BANNERS_SORT_ORDER; ?></td>
            <td class="main"><?php echo tep_draw_input_field('banners_sort_order', $bInfo->sort_order, 'size="3"'); ?></td>
          </tr>
        </table> 
<?php echo TEXT_BANNERS_BANNER_NOTE . '<br>' . TEXT_BANNERS_INSERT_NOTE . '<br>' . TEXT_BANNERS_EXPIRCY_NOTE . '<br>' . TEXT_BANNERS_SCHEDULE_NOTE; ?>
   ...
   <p>
<?php echo (($form_action == 'insert') ? tep_image_submit('button_insert.png', IMAGE_INSERT) : tep_image_submit('button_update.png', IMAGE_UPDATE)). ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_BANNER_MANAGER, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . (isset($_GET['bID']) ? 'bID=' . $_GET['bID'] : '')) . '">' .  IMAGE_CANCEL . '</a>'; ?>
          ...
          <tr>
            <td valign="top"><table class="table table-hover table-condensed table-responsive">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_STATISTICS; ?></td>
...
<?php
// Banner Rotator;
    $banners_query_raw = "select language_id, banners_id, banners_title, banners_image, banners_group, status, expires_date, expires_impressions, date_status_change, date_scheduled, date_added from " . TABLE_BANNERS . " order by sort_order, banners_title, banners_group";
...
?>
                <td class="dataTableContent"><?php echo '<a href="javascript:popupImageWindow(\'' . FILENAME_POPUP_IMAGE . '?banner=' . $banners['banners_id'] . '\')"><i class="fa fa-eye text-info"></i> </a>&nbsp;' . $banners['banners_title']; ?></td>
                <td class="dataTableContent"><?php echo $banners['banners_group']; ?></td>
                <td class="dataTableContent"><?php echo $banners_shown . ' / ' . $banners_clicked; ?></td>
                <td class="dataTableContent">
<?php
      if ($banners['status'] == '1') {
        echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=0') . '"><i class="fa fa-check-circle-o text-success"></i> </a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=1') . '"><i class="fa fa-times-circle-o text-danger"></i> </a>';
      }
?></td>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_BANNER_STATISTICS, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id']) . '"><i class="fa fa-bar-chart-o text-success"></i> </a> '; if (isset($bInfo) && is_object($bInfo) && ($banners['banners_id'] == $bInfo->banners_id)) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $banners['banners_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  ...
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'action=new') . '">' . IMAGE_NEW_BANNER . '</a>'; ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . '&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $_GET['bID']) . '">' . IMAGE_CANCEL . '</a>');
      break;
    default:
      if (is_object($bInfo)) {
        $heading[] = array('text' => '<b>' . $bInfo->banners_title . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $bInfo->banners_id . '&action=new') . '">' .  IMAGE_EDIT . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $bInfo->banners_id . '&action=delete') . '">' .  IMAGE_DELETE . '</a>');
        // Start Banner Rotator;
        $language_query_raw = "
          select
            name
