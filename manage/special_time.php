...
?>
                ...
                <td class="dataTableContent" width="156"><?php echo $delivery_time['em_max_limit']; ?></td>
                <td class="dataTableContent"><?php if (isset($cInfo) && is_object($cInfo) && ($delivery_time['id'] == $cInfo->id) ) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_EMERGENCY_DELIVERY_TIME, 'page=' . $_GET['page'] . '&dayid=' . $_GET['dayid'].'&cID=' . $delivery_time['id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
...
?>
                  <tr>
                    <td colspan="3" align="right"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_EMERGENCY_DELIVERY_TIME, 'page=' . $_GET['page'] . '&action=new') . '">New Delivery Time</a>'; ?></td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_UPDATE) . '&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_EMERGENCY_DELIVERY_TIME, 'page=' . $_GET['page'] . '&cID=' . $cInfo->id) . '">' .  IMAGE_CANCEL. '</a>');
      break;
  	  case 'new':
	...
	$costs[3] = array('id' => 5, 'text' =>'5');
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . '&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_EMERGENCY_DELIVERY_TIME, 'page=' . $_GET['page']) . '">' . IMAGE_CANCEL . '</a>');
      break;
      case 'edit':
	...
	$costs[3] = array('id' => 5, 'text' =>'5');
      ...
      $contents[] = array('text' => '<br>' . TEXT_INFO_EDIT_MAX_LIMIT . '<br>' . tep_draw_input_field('em_max_limit', $cInfo->em_max_limit));
          $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . '&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_EMERGENCY_DELIVERY_TIME, 'page=' . $_GET['page'] . '&cID=' . $cInfo->id) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    default:
      if (is_object($cInfo)) {
        $heading[] = array('text' => '<b>' .$cInfo->delv_date.'('.$cInfo->slot . ')</b>');

        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_EMERGENCY_DELIVERY_TIME, 'page=' . $_GET['page'] . '&dayid=' . $_GET['dayid'] . '&cID=' . $cInfo->id . '&action=edit') . '">' .IMAGE_EDIT . '</a>&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_EMERGENCY_DELIVERY_TIME, 'page=' . $_GET['page'] . '&cID=' . $cInfo->id . '&action=delete') . '">' .  IMAGE_DELETE . '</a>');
