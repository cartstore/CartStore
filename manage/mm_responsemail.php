<?php
/*
  $Id: mm_responsemail.php 2011-06-30 mail_manager module  http://www.css-oscommerce.com
        ...
        <td>
        	<table border="0" width="100%" cellspacing="0" cellpadding="2" >
          		<tr>
            		...
            		<?php echo (($form_action == 'insert') ? tep_image_submit('button_save.gif', IMAGE_SAVE) : tep_image_submit('button_update.gif', IMAGE_UPDATE)). '&nbsp;&nbsp;<a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . (isset($HTTP_GET_VARS['nID']) ? 'nID=' . $HTTP_GET_VARS['nID'] : '')) . '">CANCEL</a>'; ?>
            		</td>
          		</tr>
        	</table>
        </td>
...
<?php
  ...
  if ( ($HTTP_GET_VARS['flag'] == '0') || ($HTTP_GET_VARS['flag'] == '1') ) {
          ...
          if (USE_CACHE == 'true') {
            ...
            echo '<tr><td class="main">Status changed to: <strong>'.$newstatus.'</strong><a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' .$HTTP_GET_VARS['nID']) . '">BACK</a></td></tr>';    	
...
?>
      <tr><td align="right"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $HTTP_GET_VARS['nID']) . '">BACK</a>'; ?></td></tr>
...
<?php
	...
	?>
    	...
    	<tr>
    		<td><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $HTTP_GET_VARS['nID']) . '">'; ?>BACK</a></td></tr>
      	...
      	<tr>
        	<td>
        		<table border="0" width="100%" cellspacing="0" cellpadding="0"  >
          			<tr>
            			<td valign="top">
            				<table border="0" width="100%" cellspacing="0"  class="table table-hover" >
      ...
      if (isset($nInfo) && is_object($nInfo) && ($mail['mail_id'] == $nInfo->mail_id) ) {
       echo '<tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->mail_id . '&action=preview') . '\'">' . "\n";      
        ...
        echo '<tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $mail['mail_id']) . '\'">' . "\n";
							...
							?>             
                					<td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $mail['mail_id'] . '&action=preview') . '"><i class="fa fa-eye"></i></a>&nbsp;' . $mail['title']; ?></td>
                					...
                					<td class="dataTableContent" align="right">
                						<?php 
                							if ($mail['status'] == '1') {
        									echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'action=setflag&flag=0&nID=' . $mail['mail_id']) . '"><i class="fa fa-check-circle-o text-success"></i></a>';
      										} else {
       								 		echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'action=setflag&flag=1&nID=' . $mail['mail_id']) . '"><i class="fa fa-times-circle-o text-danger"></i>';
     									 		}               					               			
       						?>
       								 		</td>                			
                					<td class="dataTableContent" align="right"><?php if (isset($nInfo) && is_object($nInfo) && ($mail['mail_id'] == $nInfo->mail_id) ) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $mail['mail_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              					</tr>
	<?php
    	}
	?>
                					...
                					<td colspan="6">
                  						<table border="0" width="100%" cellspacing="0" cellpadding="2">
                  							...
                  							<tr>
                    							...
                    							<?php
                    							// uncomment to activate. Commenting this out prevents installing a new newsletter 
                    							//	echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'action=new') . '">' . tep_image_button('button_new_newsletter.gif', IMAGE_NEW_NEWSLETTER) . '</a>'; 
                    							?>
                    							</td>
                  							</tr>
                						</table>
...
switch ($action) {
    ...
    case 'reset':
      ...
      $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->mail_id . '&action='.$confirm_action) . '">CONFIRM</a>
      								<a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $HTTP_GET_VARS['nID']) . '">CANCEL</a>');
    break;
    default:
      if (is_object($nInfo)) {
        $heading[] = array('text' => '<b>' . $nInfo->title . '</b>');  
          $contents[] = array('align' => 'center', 'text' => '
          				<a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->mail_id . '&action=new') . '">EDIT </a> 
          				<a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->mail_id . '&action=preview') . '">PREVIEW</a>           							       				
          				<table><tr><td class="main"><a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->mail_id . '&action=restore') . '">RESTORE</a>           				          				         				
          				</td></tr><tr><td class="main">'.TEXT_BACKUP_MESSAGE.'
          				</td></tr></table><table><tr><td class="main"><a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->mail_id . '&action=reset') . '">RESET</a>           				          				
          				</td></tr><tr><td class="main">'.TEXT_RESET_MESSAGE.'
          				</td></tr></table><table><tr><td class="main"><a class="btn btn-default" href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $HTTP_GET_VARS['page'] . '&nID=' . $nInfo->mail_id . '&action=test') . '">Test</a>
