<?php
/*
  $Id: affiliate_affiliates.php,v 2.00 2003/10/12
...
<?php echo tep_draw_form('affiliate', FILENAME_AFFILIATE, tep_get_all_get_params(array('action')) . 'action=update', 'post', 'onSubmit="return check_form();"'); ?>
 
 
 <h3><?php echo CATEGORY_PERSONAL; ?> </h3> <table class="table">
<?php
...
?>
    <h3>  <?php echo CATEGORY_COMMISSION; ?></h3>
...
<p>
<?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('action'))) .'">' . IMAGE_CANCEL . '</a>';?>
...
<?php echo HEADING_TITLE_SEARCH . '</label> ' . tep_draw_input_field('search'); ?>
</div>
          </form>
          ...
          <tr>
            <td valign="top"><table class="table table-hover table-condensed table-responsive">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_COMMISSION; ?></td>
...
?>
                ...
                <td class="dataTableContent"><?php if($affiliate['affiliate_commission_percent'] > AFFILIATE_PERCENT) echo $affiliate['affiliate_commission_percent']; else echo  AFFILIATE_PERCENT; ?> %</td>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('acID', 'action')) . 'acID=' . $affiliate['affiliate_id'] . '&action=edit') . '"><i class="fa fa-pencil text-info"></i> </a>'; echo '<a href="' . $affiliate['affiliate_homepage'] . '" target="_blank">' . $affiliate['affiliate_homepage'] . '</a>'; ?></td>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_STATISTICS, tep_get_all_get_params(array('acID')) . 'acID=' . $affiliate['affiliate_id']) . '"><i class="fa fa-bar-chart-o text-info"></i> </a>&nbsp;'; if ( (is_object($aInfo)) && ($affiliate['affiliate_id'] == $aInfo->affiliate_id) ) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('acID')) . 'acID=' . $affiliate['affiliate_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
  ...
  switch ($_GET['action']) {
    ...
    default:
      if (is_object($aInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('acID', 'action')) . 'acID=' . $aInfo->affiliate_id . '&action=edit') . '">' . IMAGE_EDIT . '</a> <a  class="btn btn-default"  href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('acID', 'action')) . 'acID=' . $aInfo->affiliate_id . '&action=confirm') . '">' . IMAGE_DELETE . '</a> <a  class="btn btn-default"  href="' . tep_href_link(FILENAME_AFFILIATE_CONTACT, 'selected_box=affiliate&affiliate=' . $aInfo->affiliate_email_address) . '">' . IMAGE_EMAIL . '</a>');
