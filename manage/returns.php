...
if ($return_complete['returns_date_finished'] != '0000-00-00 00:00:00') {
 ?>
  <?php echo HEADING_TITLE; ?>
      ...
      <?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('action'))) . '">' .  IMAGE_BACK . '</a>'; ?>
  ...
  <tr>
    <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
        <tr class="dataTableHeadingRow">
          ...
          <td class="dataTableHeadingContent">Deductions</td>
          <td class="dataTableHeadingContent">Refund Amount</td>
        </tr>
        <?php
// for ($i=0; $i<sizeof($order->products); $i++) {
      ...
      echo '            </td>' . "\n" .
           ...
           '            <td class="dataTableContent" valign="top">' . tep_display_tax_value($order->products['tax']) . '%</td>' . "\n" .
           '            <td class="dataTableContent" valign="top"><b>' . $currencies->format($order->products['final_price'] * $order->products['qty'], true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n" .
           '            <td class="dataTableContent" valign="top"><b>' . $currencies->format(tep_add_tax($order->products['final_price'], $order->products['tax']) * $order->products['qty'], true, $order->info['currency'], $order->info['currency_value']) . '</b></td>' . "\n" .
            '            <td class="dataTableContent" valign="top"><b>' . $currencies->format($refund['refund_payment_deductions']) . '</b></td>' . "\n" .
           '            <td class="dataTableContent" valign="top"><b>' . $currencies->format($refund['refund_payment_value']) . '</b></td>' . "\n";
       echo '          </tr>' . "\n";
//}
?>
        <tr>
  ...
  <tr>
    <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td class="pageHeading" align="right"><?php echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('action'))) . '">' .  IMAGE_BACK . '</a>'; ?></td>
        </tr>
      </table></td>
  </tr>
  <tr>
...
<tr>
  <td colspan="2" align="right"><?php echo
				...
				'<a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS_INVOICE, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . IMAGE_ORDERS_INVOICE . '</a> <a class="btn btn-default" href="' . tep_href_link("returns_packingslip.php", 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . IMAGE_ORDERS_PACKINGSLIP . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('action'))) . '">' .  IMAGE_BACK . '</a>'; ?></td>
</tr>
<?php
...
<?php echo '<label>'. HEADING_TITLE_SEARCH . '</label> ' . tep_draw_input_field('cID', '', 'size="15"') . tep_draw_hidden_field('action', 'edit'); ?>
              </form></div>
           
              
  <div class="form-group ">
                <?php echo tep_draw_form('status', FILENAME_RETURNS, '', 'get'); ?>
     
                    <?php echo '<label>'. HEADING_TITLE_STATUS . '</label> ' . tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => 'All Returns')), $orders_statuses), '', 'onChange="this.form.submit();"'); ?>
      ...
      <tr>
        <td valign="top"><table class="table">
            ...
            <tr class="dataTableHeadingRow">
              <?php // ADDED BY BURT ?>
              <td class="dataTableHeadingContent">RMA #</td>
              <?php // END BURT ?>
...
?>
            <?php // ADDED BY BURT ?>
            <td class="dataTableContent"><b><?php echo $orders['rma_value']; ?></b></a>
                <?php // END BURT ?>
              ...
              <td class="dataTableContent"><?php echo $currencies->format($orders['final_price']); ?></td>
              <td class="dataTableContent"><?php echo tep_datetime_short($orders['date_purchased']); ?></td>
              <td class="dataTableContent"><?php echo $orders['return_reason_name']; ?></td>
              <td class="dataTableContent"><?php echo $orders['returns_status_name'];?></td>
            </tr><?php
    } // WHILE LOOP
?>
            <tr>
  ...
  switch ($_GET['action']) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->returns_id) . '">' . IMAGE_CANCEL . '</a>');
      break;
    default:
      if (is_object($oInfo)) {
        $heading[] = array('text' => '<b>[' . $oInfo->returns_id . ']&nbsp;&nbsp;' . tep_datetime_short($oInfo->date_purchased) . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->returns_id . '&action=edit') . '">' .  IMAGE_EDIT . '</a> <a class="btn btn-default" href="' . tep_href_link("returns_packingslip.php", 'oID=' . $oInfo->returns_id) . '" TARGET="_blank">' .  IMAGE_ORDERS_PACKINGSLIP . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->returns_id . '&action=delete') . '">' .  IMAGE_DELETE . '</a>');
        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_RETURNS_INVOICE, 'oID=' . $oInfo->returns_id) . '" TARGET="_blank">' .  IMAGE_ORDERS_INVOICE .
