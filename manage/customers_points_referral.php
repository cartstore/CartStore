...
?>
 <div class="page-header"><h1>   <a class="pull-right" data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
  ...
  echo HEADING_TITLE . '</h1><span class="label label-success">' . HEADING_RATE . ' ' . HEADING_AWARDS . $currencies->format(1) . ' = ' . number_format(POINTS_PER_AMOUNT_PURCHASE, POINTS_DECIMAL_PLACES) . ' ' . $point_or_points . '</span> <span class="label label-success"> ' . HEADING_REDEEM . number_format(POINTS_PER_AMOUNT_PURCHASE, POINTS_DECIMAL_PLACES) . ' ' . $point_or_points . ' = ' . $currencies->format(POINTS_PER_AMOUNT_PURCHASE * REDEEM_POINT_VALUE) .'</span>';
?></small></div>
        <div class="panel-group" id="accordion">
                  ...
                  <div class="panel panel-default">
                      ...
                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body"><i class="fa fa-comment-o fa-5x pull-left"></i>
...
<?php
  echo HEADING_TITLE_SEARCH . ' </label>' . tep_draw_input_field('search', '', 'size="12"');
?>
</div>

<div class="form-group"><label>
<?php
  echo TABLE_HEADING_POINTS_TYPE . '</label> ' . tep_draw_pull_down_menu('potype', $potype_array) . '</div>

  
  
 <div class="form-group"><label> ' . TABLE_HEADING_POINTS_STATUS . '</label>' . tep_draw_pull_down_menu('filter', $filter_array, '', 'onChange="this.form.submit();"');
?>
</div>
                      </form>
...
?></td>
  ...
  while ($pending_points = tep_db_fetch_array($pending_points_query)) {
      if ($pending_points['points_type'] == 'RF') {
          ...
          $link = '<a href="' . tep_href_link(FILENAME_ORDERS . '?oID=' . $pending_points['orders_id'] . '&action=edit') . '"><i class="fa fa-pencil text-info"></i> </a> ';
      }
      if ($pending_points['points_type'] == 'RV') {
          ...
          $link = '<a href="' . tep_href_link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $reviews['reviews_id'] . '&action=edit') . '"><i class="fa fa-pencil text-info"></i></a>';
      }
...
?></td>
      ...
      if (isset($uInfo) && is_object($uInfo) && ($pending_points['unique_id'] == $uInfo->unique_id)) {
          ...
          echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID')) . 'uID=' . $pending_points['unique_id']) . '"><i class="fa fa-hand-o-up"></i></a>';
      }
?>
                        </td>
                    </tr>
...
?>
      ...
      echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL) . '">' . IMAGE_RESET . '</a>';
?></td>
                          </tr>
                          <?php
  }
?>
  ...
  switch ($action) {
      case 'confirm':
          ...
          $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_confirm_points.gif', BUTTON_TEXT_CONFIRM_PENDING_POINTS) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id) . '">' . IMAGE_CANCEL . '</a>');
          break;
      case 'cancel':
          ...
          $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_cancel_points.gif', BUTTON_TEXT_CANCEL_PENDING_POINTS) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id) . '">' . IMAGE_CANCEL . '</a>');
          break;
      case 'adjust':
          ...
          $contents[] = array('text' => $value_field);
          $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_adjust_points.gif', BUTTON_TEXT_ADJUST_POINTS) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id) . '">' . IMAGE_CANCEL . '</a>');
          break;
      case 'rollback':
          ...
          $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_rollback_points.gif', BUTTON_TEXT_ROLL_POINTS) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id) . '">' . IMAGE_CANCEL . '</a>');
          break;
      case 'delete':
          ...
          $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', BUTTON_TEXT_REMOVE_RECORD) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id) . '">' . IMAGE_CANCEL . '</a>');
          break;
      default:
          if (isset($uInfo) && is_object($uInfo)) {
              ...
              if ($uInfo->points_status == 1) {
                  $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=confirm') . '">' . BUTTON_TEXT_CONFIRM_PENDING_POINTS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=cancel') . '">' . BUTTON_TEXT_CANCEL_PENDING_POINTS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=adjust') . '">' . BUTTON_TEXT_ADJUST_POINTS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $uInfo->customers_email_address) . '">' . IMAGE_EMAIL . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=delete') . '">' . BUTTON_TEXT_REMOVE_RECORD . '</a>');
              }
              if ($uInfo->points_status == 2) {
                  $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('action')) . 'uID=' . $uInfo->unique_id . '&action=rollback') . '">' . BUTTON_TEXT_ROLL_POINTS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $uInfo->customers_email_address) . '">' . IMAGE_EMAIL . '</a> <a class="btn btn-default"  href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=delete') . '">' . BUTTON_TEXT_REMOVE_RECORD . '</a>');
              }
              if ($uInfo->points_status == 3) {
                  $contents[] = array('text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=confirm') . '">' . BUTTON_TEXT_CONFIRM_PENDING_POINTS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=adjust') . '">' . BUTTON_TEXT_ADJUST_POINTS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_ORDERS . '?oID=' . $uInfo->orders_id . '&action=edit') . '">' . IMAGE_DETAILS . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $uInfo->customers_email_address) . '">' . MAGE_EMAIL . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_CUSTOMERS_POINTS_REFERRAL, tep_get_all_get_params(array('uID', 'action')) . 'uID=' . $uInfo->unique_id . '&action=delete') . '">' . BUTTON_TEXT_REMOVE_RECORD . '</a>');
              }
              ...
              if ($uInfo->points_type == 'RF') {
                  $order_link = '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $uInfo->orders_id . '&action=edit') . '"><i class="fa fa-pencil text-info"></i> </a>';
              ...
              if ($uInfo->points_type == 'RV') {
                  $review_link = '<a href="' . tep_href_link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $uInfo->reviews_id . '&action=edit') . '"></a>';
                  $contents[] = array('text' => '<br><b>' . TEXT_INFO_POINTS_COMMENT . '</b><br>' . $uInfo->points_comment);
