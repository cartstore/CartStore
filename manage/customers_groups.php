...
On this screen you can create customer groups, then you can assign customers to groups. The groups feature enables  you to define special pricing per customer groups and in the near future define if a product or article only shows to a certain customer group. Additionally this hide/show based on customer group feature is to be extended to specific modules as well.                       </div>
                      </div> 
                  </div>   
              </div>   
<?php echo tep_draw_form('customers', 'customers_groups.php', tep_get_all_get_params(array('action')) . 'action=update', 'post', 'onSubmit="return check_form();"'); ?>
...
?>
      ...
      <tr>
        <td align="right" class="main"><?php echo tep_image_submit('button_update.png', IMAGE_UPDATE) . ' <a class="btn btn-default" href="' . tep_href_link('customers_groups.php', tep_get_all_get_params(array('action','cID'))) .'">' .  IMAGE_CANCEL . '</a>'; ?></td>
      </tr>
      </form>
...
On this screen you can create customer groups, then you can assign customers to groups. The groups feature enables  you to define special pricing per customer groups and in the near future define if a product or article only shows to a certain customer group. Additionally this hide/show based on customer group feature is to be extended to specific modules as well.                         </div>
    ...
    <?php echo tep_draw_form('customers', 'customers_groups.php', tep_get_all_get_params(array('action')) . 'action=newconfirm', 'post', 'onSubmit="return check_form();"'); ?>
...
<table class="table table-condensed">
          ...
          <tr>
            <td class="main">&#160;</td>
            <td class="main"> is set to true in the Configuration for your store and Tax Exempt (below) to 'No'.</td>
          </tr>
          <tr>
            <td class="main"><?php echo ENTRY_GROUP_TAX_EXEMPT; ?></td>
            <td class="main"><?php
 ...
 <?php include_once(DIR_WS_LANGUAGES . $language . '/modules.php');
	echo '<h3>'. HEADING_TITLE_MODULES_PAYMENT . '</h3>'; ?> 
<table  class="table">
	   <tr class="success">
            <td class="success"><?php echo tep_draw_radio_field('group_payment_settings', '1', false, '0') . ' ' . ENTRY_GROUP_PAYMENT_SET . ' ' . tep_draw_radio_field('group_payment_settings', '0', false, '0') . ' ' . ENTRY_GROUP_PAYMENT_DEFAULT ; ?> 
          </tr>
<?php
  $module_active = explode (";",MODULE_PAYMENT_INSTALLED);
  $ship_module_active = explode (";",MODULE_SHIPPING_INSTALLED);
...
?>
	   <tr>
            <td class="main"><div class="alert alert-info"><?php echo ENTRY_PAYMENT_SET_EXPLAIN ?></div></td>
           </tr>
        </table>
...
<table class="table">
	   <tr>
            <td class="success"><?php echo tep_draw_radio_field('group_shipment_settings', '1', false, (tep_not_null($cInfo->group_shipment_allowed)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_SHIPPING_SET . '&nbsp;&nbsp;' . tep_draw_radio_field('group_shipment_settings', '0', false, (tep_not_null($cInfo->group_shipment_allowed)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_SHIPPING_DEFAULT ; ?></td>
          </tr>
<?php
    $ship_module_active = explode (";",MODULE_SHIPPING_INSTALLED);
    $installed_shipping_modules = array();
...
?>
	   <tr>
            <td class="main"><div class="alert alert-info"><?php echo ENTRY_SHIPPING_SET_EXPLAIN ?></div></td>
           </tr>
        </table>
       
       <p>
<?php echo tep_image_submit('button_update.png', IMAGE_UPDATE) . ' <a class="btn btn-default" href="' . tep_href_link('customers_groups.php', tep_get_all_get_params(array('action','cID'))) .'">' .  IMAGE_CANCEL . '</a>'; ?>
...
<?php echo tep_draw_form('search', 'customers_groups.php', '', 'get'); ?>
...
<div class="form-group hide"><label><?php echo HEADING_TITLE_SEARCH . '</label> ' . tep_draw_input_field('search'); ?></div>
	     ...
	     <table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
	       ...
	       <td class="dataTableHeadingContent"><a href="<?php echo "$PHP_SELF?listing=group"; ?>"></a> <a href="<?php echo "$PHP_SELF?listing=group-desc"; ?>"></a><?php echo TABLE_HEADING_NAME; ?></td>
...
?>
                ...
                <td class="dataTableContent"><?php if ( (is_object($cInfo)) && ($customers_groups['customers_group_id'] == $cInfo->customers_group_id) ) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link('customers_groups.php', tep_get_all_get_params(array('cID')) . 'cID=' . $customers_groups['customers_group_id']) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
?>
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a  class="btn btn-default" href="' . tep_href_link('customers_groups.php') . '">' . IMAGE_RESET . '</a>'; ?></td>
                  </tr>
<?php
    } else {
?>
			      <tr>
                    <td align="right" colspan="2" class="smallText"><?php echo '<a class="btn btn-default" href="' . tep_href_link('customers_groups.php', 'page=' . $_GET['page'] . '&action=new') . '">' .  IMAGE_INSERT . '</a>'; ?></td>
                  </tr>
<?php
	}
?>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'confirm':
        if ($_GET['cID'] != '0') {
            $heading[] = array('text' => '<br><b>' . TEXT_INFO_HEADING_DELETE_GROUP . '</b>');
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a class="btn btn-default" href="' . tep_href_link('customers_groups.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_group_id) . '">' . IMAGE_CANCEL . '</a>');
        } else {
            $heading[] = array('text' => '<br><b>' . TEXT_INFO_HEADING_DELETE_GROUP . '</b>');
    ...
    default:
      if (is_object($cInfo)) {
        $heading[] = array('text' => '<br><b>' . $cInfo->customers_group_name . '</b>');
        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link('customers_groups.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_group_id . '&action=edit') . '">' .  IMAGE_EDIT . '</a>' . ($cInfo->customers_group_id != 0 ? '<a class="btn btn-default" href="' . tep_href_link('customers_groups.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_group_id . '&action=confirm') . '">' .  IMAGE_DELETE . '</a>' : ''));
