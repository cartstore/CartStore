...
<div class="page-header"><h1><?php echo HEADING_TITLE_MODULES_SHIPPING; ?></h1></div>
...
?>
                ...
                <td class="dataTableContent"><?php if (is_numeric($module->sort_order($vendors_id))) echo $module->sort_order($vendors_id); ?></td>
                <td class="dataTableContent"><?php if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) { echo '<i class="fa fa-long-arrow-right"></i>'; } else { echo '<a href="' . tep_href_link(FILENAME_VENDOR_MODULES, 'module=' . $class . '&vendors_id=' . $vendors_id) . '"><i class="fa fa-hand-o-up"></i></a>'; } ?> </td>
              </tr>
<?php
    }
  }
...
<?php
  ...
  switch ($action) {
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.png', IMAGE_UPDATE) . ' <a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDOR_MODULES, 'module=' . $_GET['module'] . '&vendors_id=' . $vendors_id) . '">' .  IMAGE_CANCEL . '</a>');
      break;
    default:
      $heading[] = array('text' => '<b>' . $mInfo->title . '</b>');
      ...
      if ($mInfo->status == '1') {
        ...
        $keys = substr($keys, 0, strrpos($keys, '<br><br>'));
        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDOR_MODULES, 'module=' . $mInfo->code . '&action=remove&vendors_id=' . $vendors_id) . '">' . IMAGE_MODULE_REMOVE . '</a> <a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDOR_MODULES, (isset($_GET['module']) ? '&module=' . $_GET['module'] : '') . '&action=edit&vendors_id=' . $vendors_id) . '">' .  IMAGE_EDIT . '</a>');
        $contents[] = array('text' => '<br>' . $mInfo->description);
        $contents[] = array('text' => '<br>' . $keys);
      } else {
        $contents[] = array('align' => 'center', 'text' => '<a class="btn btn-default" href="' . tep_href_link(FILENAME_VENDOR_MODULES, 'module=' . $mInfo->code . '&action=install&vendors_id=' . $vendors_id) . '">' . IMAGE_MODULE_INSTALL . '</a>');
        $contents[] = array('text' => '<br>' . $mInfo->description);
      }
      break;
  }
