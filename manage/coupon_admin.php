...
<i class="fa fa-pencil fa-5x pull-left"></i>
ssss                      </div>
                  </div>   
              </div>    
...
?></td>
              ...
              if ((is_object($cInfo)) && ($cc_list['unique_id'] == $cInfo->unique_id)) {
                  ...
                  echo '<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'page=' . $_GET['page'] . '&cid=' . $cc_list['coupon_id']) . '"><i class="fa fa-hand-o-up"></i></a>';
              }
?>
                  &nbsp;</td>
              </tr>
...
?>
                          &nbsp;</td>
                        <td align="right"><?php //
        //  echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_COUPON_ADMIN) . '">' . IMAGE_CANCEL . '</a> ' . tep_image_submit('button_send_mail.png', IMAGE_SEND_EMAIL);
?></td>
                      </tr>
                    </table></td>
                </tr>
...
?>
                  ...
                  <div class="panel panel-default">
                      ...
                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body">
<i class="fa fa-pencil fa-5x pull-left"></i>
Please refer to the right column help info per field.                                 </div>
                      </div>
                  </div>   
              </div>    
                    <?php
...
?></td>
                        <td align="left"><?php
                  echo '<a class="btn btn-default" href="' . tep_href_link('coupon_admin.php', '');
?>
                          ">
                          <?php
                  echo  IMAGE_CANCEL;
...
?>
      ...
      <div class="panel-group" id="accordion">
                  ...
                  <div class="panel panel-default">
                      ...
                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body">
<i class="fa fa-tags fa-5x pull-left"></i>
The discount coupons screen allows you to create and delete coupons which you then may send to others so that they may receive discounts in checkout.                                 </div>
                                 </div>
                      </div>
                  </div>   
              </div>    
...
?>
                  ...
                  echo '<div class="form-group"><label>' . HEADING_TITLE_STATUS . ' </label>' . tep_draw_pull_down_menu('status', $status_array, $status, 'onChange="this.form.submit();"').'</div>';
?>
                            </form>
                           <table border="0" width="100%" cellspacing="0" cellpadding="0">
                        <tr>
...
?>
                      ...
                      if ($cc_list['coupon_active'] == 'Y') {
                          echo ' <a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'action=setflag&flag=N&cid=' . $cc_list['coupon_id']) . '"><i class="fa fa-check-circle-o text-success"></i></a>';
                      } else {
                          echo '<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'action=setflag&flag=Y&cid=' . $cc_list['coupon_id']) . '"><i class="fa fa-times-circle-o text-danger"></i></a>';
                      }
?></td>
                                <td class="dataTableContent"><?php
                      if ((is_object($cInfo)) && ($cc_list['coupon_id'] == $cInfo->coupon_id)) {
...
?>
                  ...
                  echo '<a class="btn btn-default" href="' . tep_href_link('coupon_admin.php', 'page=' . $_GET['page'] . '&cID=' . $cInfo->coupon_id . '&action=new') . '">' .  IMAGE_INSERT . '</a>';
?></td>
                                    </tr>
                                  </table></td>
                              </tr>
                  ...
                  switch ($_GET['action']) {
                      ...
                      default:
                          ...
                          if ($_GET['action'] == 'voucherdelete') {
                              $contents[] = array('text' => TEXT_CONFIRM_DELETE . '</br></br>' . '<a class="btn btn-default" href="' . tep_href_link('coupon_admin.php', 'action=confirmdelete&status=' . $status . (($_GET['page'] > 1) ? '&page=' . $_GET['page'] : '') . '&cid=' . $_GET['cid'], 'NONSSL') . '">' .  IMAGE_CONFIRM . '</a>' . '<a class="btn btn-default" href="' . tep_href_link('coupon_admin.php', 'cid=' . $cInfo->coupon_id, 'NONSSL') . '">' .  IMAGE_CANCEL . '</a>');
                          } else {
                              $prod_details = NONE;
