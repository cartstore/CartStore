...
$custom_fields[TABLE_PRODUCTS_SHIPPING] = array('products_ship_zip' => 'products_ship_zip', 'products_ship_price' => 'products_ship_price', 'products_ship_price_two' => 'products_ship_price_two');
//
...
foreach ($custom_fields[TABLE_PRODUCTS_SHIPPING] as $key => $name) {
  $default_these[] = 'v_' . $key;
}
...
if ( !empty($_GET['download']) && ($_GET['download'] == 'stream' or $_GET['download'] == 'activestream' or $_GET['download'] == 'tempfile') ){
    ...
    while ($row){
        ...
        // If you have other modules that need to be available, put them here
		    // if the filelayout says we need a manufacturers name, get it
        if (isset($filelayout['v_products_ship_zip'])){
            if ($row['v_products_id'] != ''){
                $sql2 = "SELECT *
...
CartStore Mass Product Import</h1></div>
        ...
        <div class="panel-group" id="accordion">
                  ...
                  <div class="panel panel-default">
                      ...
                      <div id="collapseOne" class="panel-collapse collapse">
                          <div class="panel-body"><i class="fa fa-file-code-o fa-5x pull-left"></i>
Help for this section is not yet available.                          </div>
                      </div>
                  </div>   
              </div>    
              <div class="alert alert-danger">  <p> <b>Warning for advanced users only.</b> For professional use.</p> </div>
...
<p>Please backup your database before performing any operations!</p></div>

 

<div class="alert alert-warning" <p>We accept <b>no liability</b> for damage done using this tool. This tool is for experienced professionals only.</p></div>
...
?>
           ...
           <form enctype="multipart/form-data" action="easypopulate.php?split=0<?php if (defined('SID') && tep_not_null(SID)) { echo '&'.tep_session_name().'='.tep_session_id(); } ?>" method="post"><?php if (defined('SID') && tep_not_null(SID)) { echo tep_draw_hidden_field(tep_session_name(), tep_session_id()); } ?>
               <div class="form-group"><h3>Upload and Import EP File</h3>
                  <input type="hidden" name="MAX_FILE_SIZE" value="100000000">
                  <input class="form-control" name="usrfl" type="file" size="50">
                  
                  </div>
                  <div class="form-group"> 
                  <select name="imput_mode"  class="form-control" >
                  ...
                  <p>
                  <input class="btn btn-default" type="submit"  name="buttoninsert" value="Insert into db"></p>
              </form>
<hr />
           <form enctype="multipart/form-data" action="easypopulate.php<?php if (defined('SID') && tep_not_null(SID)) { echo '?'.tep_session_name().'='.tep_session_id(); } ?>" method="post"><?php if (defined('SID') && tep_not_null(SID)) { echo tep_draw_hidden_field(tep_session_name(), tep_session_id()); } ?>
             <div class="form-group"><h3>Import EP file from Temp Dir</h3>
                  <?PHP
                    ...
                    if($array_size == 0){
                      echo ('            <input class="form-control" type="text" name="localfile" size="50">' . "\n");
                    } else {
                      echo ('            <select class="form-control"  name="localfile" size="1">' . "\n");
                  ...
                  <p>
                  <input type="submit" class="btn btn-default"  name="buttoninsert" value="Insert into db"></p>
               
             </form>
<hr />
 
<h3>Create then Download Files</h3> 
   <p>  Create entire file in server memory then stream download after completed.</p>
        <ul>
        <li><a href="easypopulate.php?download=stream&dltype=full<?php if (defined('SID') && tep_not_null(SID)) { echo '&'.tep_session_name().'='.tep_session_id(); } ?>">Download <b>Complete<?php if (EP_SPPC_SUPPORT == true) { echo ' w/SPPC'; } ?></b> <?php echo ((EP_EXCEL_SAFE_OUTPUT == true)?".csv":".txt"); ?> file to edit</a></li>
        <li><?php if (EP_EXTRA_FIELDS_SUPPORT == true) { ?>
          <a href="easypopulate.php?download=stream&dltype=extra_fields<?php if (defined('SID') && tep_not_null(SID)) { echo '&'.tep_session_name().'='.tep_session_id(); } ?>">Download <b>Extra Fields</b> <?php echo ((EP_EXCEL_SAFE_OUTPUT == true)?".csv":".txt"); ?> file to edit</a> 
<?php } ?></li>
        </ul>
       ...
       <h3>Create Files in Temp Dir</h3> 
        <p> Create entire file in server memory then save to Temp Dir after completed. </p>
        
        <p>  <a href="easypopulate.php?download=tempfile&dltype=full<?php if (defined('SID') && tep_not_null(SID)) { echo '&'.tep_session_name().'='.tep_session_id(); } ?>">Create <b>Complete</b> <?php echo ((EP_EXCEL_SAFE_OUTPUT == true)?".csv":".txt"); ?> file in temp dir</a> 
          <!-- VJ product attributes begin //-->
          <?php if (EP_PRODUCTS_WITH_ATTRIBUTES == true) { ?>
          <?php } ?>
          <!-- VJ product attributes end //-->
...
function ep_create_filelayout($dltype, $attribute_options_array, $languages, $custom_fields){
	...
	$ep_additional_layout_product_shipping = '';
 ...
 foreach ($custom_fields[TABLE_SPECIALS] as $key => $name) {
		 ...
		 foreach ($custom_fields[TABLE_PRODUCTS_SHIPPING] as $key => $name) {
			$ep_additional_layout_product_shipping .= '$filelayout[\'v_' . $key . '\'] = $iii++;';
		}
    if (EP_MORE_PICS_6_SUPPORT == true) {
    ...
    case 'full':
        ...
        if (!empty($ep_additional_layout_product_shipping)) {
           eval($ep_additional_layout_product_shipping);
         }
...
//        $v_products_id    =    $items[$filelayout['v_products_id']];
    ...
    // //////////////////////////////////
        ...
        if ($process_product == true) {
         ...
         if ( isset($v_products_ship_price) && $v_products_ship_price != '' ){
		  $result = tep_db_query("SELECT products_id FROM ".TABLE_PRODUCTS_SHIPPING." WHERE products_id = '". $v_products_id . "'");
           if (tep_db_num_rows($result) == 0)  {
				$sql = "INSERT INTO ".TABLE_PRODUCTS_SHIPPING."(
                        products_id,
                        products_ship_zip,
                        products_ship_price,
                        products_ship_price_two) VALUES (
                        $v_products_id,'$v_products_ship_zip'," . (float)$v_products_ship_price . "," . (float)$v_products_ship_price_two . ")";
				$result = tep_db_query($sql);
			} else {
				$sql = "update ".TABLE_PRODUCTS_SHIPPING." set products_ship_zip='$v_products_ship_zip', products_ship_price = " . (float)$v_products_ship_price . ", products_ship_price_two = " . (float)$v_products_ship_price . "  where products_id=$v_products_id";
				$result = tep_db_query($sql);
			}
		 }
