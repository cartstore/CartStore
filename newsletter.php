		...
		if ($list_on_off != 1) {
			...
			if ($check1['total'] > 0) {
				...
				if ($check3['total'] > 0) {
					...
					$q_type = "insert";
					$sql_data_array = array('customers_firstname' => $list_firstname, 'customers_lastname' => $list_lastname, 'customers_email_address' => $list_email_add, 'customers_newsletter' => $list_on_off);
...
<table width="100%" border="0">
	<tr>
		<td>
			...
			<h1>Subscribe</h1> <?php echo tep_draw_form('maillist', 'newsletter.php', 'post', '') . tep_draw_hidden_field('action', 'process'); ?>  
		 
</div>		 		<p class="lead">You can unsubscribe at any moment by returning to this page.<p>

		<div class="form-group">
			<label>Name</label>
			<input class="form-control" type="text" name="list_firstname" value="<?php echo $list_firstname; ?>" size="20" maxlength="20" >

		</div>
		<div class="form-group">
			<label>Last name</label>
			<input class="form-control" type="text" name="list_lastname" value="<?php echo $list_lastname; ?>" size="20" maxlength="20">

		</div>
		<div class="form-group">
			<label>Email address *</label>
			<input class="form-control" type="text" name="list_email_add" value="<?php echo $list_email_add; ?>" size="20" maxlength="40">
		</div>
		<input type="radio" name="list_on_off" value=1 checked>
		<font size="-2">Subscribe</font>
		<input type="radio" name="list_on_off" value="0">
		<font size="-2">Unsubscribe</font>
		<input type="hidden" name="check" value="20">
		<input class="btn btn-default" type="submit" border="0"  name="Submit" value="Submit">
		</form> 
