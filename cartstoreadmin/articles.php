		...
		case 'update_topic' :
			...
			if (($_POST ['edit_x']) || ($_POST ['edit_y'])) {
				...
				for($i = 0, $n = sizeof ( $languages ); $i < $n; $i ++) {
					...
					'topics_description' => tep_db_prepare_input ( $_POST ['products_description'] [$language_id] ) );
	...
	if ($_GET ['action'] == 'new_topic_ACD' || $_GET ['action'] == 'edit_topic_ACD') {
      ...
      <tr>
		...
		?></td>
			...
			<tr><?php
		echo tep_draw_form ( 'new_topic', FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $_GET ['tID'] . '&action=new_topic_preview', 'post', 'enctype="multipart/form-data"' );
		?>
...
<?php
		...
		echo tep_draw_hidden_field ( 'topics_date_added', (($tInfo->date_added) ? $tInfo->date_added : date ( 'Y-m-d' )) ) . tep_draw_hidden_field ( 'parent_id', $tInfo->parent_id ) . tep_image_submit ( 'button_preview.png', IMAGE_PREVIEW ) . '&nbsp;&nbsp;<a class="button" href="' . tep_href_link ( FILENAME_ARTICLES, 'tPath=' . $tPath . '&tID=' . $_GET ['tID'] ) . '">' .  IMAGE_CANCEL  . '</a>';
		?></td>
...
<?php
    ...
    <script language="JavaScript1.2" defer>
        ...
        // MaxiDVD Added WYSIWYG HTML Area Box + Admin Function v1.7 Articles Description HTML - Head
           ...
           config.bodyStyle = 'background-color: <?php
...
-->
    ...
    <?php
		echo tep_draw_form ( 'new_article', FILENAME_ARTICLES, 'tPath=' . $tPath . (isset ( $_GET ['aID'] ) ? '&aID=' . $_GET ['aID'] : '') . '&action=article_preview', 'post', 'enctype="multipart/form-data"' );
		?>
...
<?php
		...
		for($i = 0, $n = sizeof ( $languages ); $i < $n; $i ++) {
          ...
          <tr>
			...
			echo '<span >' . tep_draw_textarea_field_ckeditor ( 'articles_head_desc_tag[' . $languages [$i] ['id'] . ']', 'soft', '70', '15', 

			(isset ( $articles_head_desc_tag [$languages [$i] ['id']] ) ? $categories_htc_description [$languages [$i] ['id']] : tep_get_articles_head_desc_tag ( $aInfo->articles_id, $languages [$i] ['id'] )) ) . ' </span>';
	...
	*/
		...
		?></td>
				...
				<tr>
					...
					<td class="main" align="right"><?php
		echo tep_draw_hidden_field ( 'articles_date_added', (tep_not_null ( $aInfo->articles_date_added ) ? $aInfo->articles_date_added : date ( 'Y-m-d' )) ) . tep_image_submit ( 'button_preview.png', IMAGE_PREVIEW ) . '&nbsp;&nbsp;<a class="button" href="' . tep_href_link ( FILENAME_ARTICLES, 'tPath=' . $tPath . (isset ( $_GET ['aID'] ) ? '&aID=' . $_GET ['aID'] : '') ) . '">' .  IMAGE_CANCEL  . '</a>';
		?></td>
...
<?php
		...
		while ( $topics = tep_db_fetch_array ( $topics_query ) ) {
			...
			'<a href="' . HTTP_SERVER . '' . DIR_WS_CATALOG . 'articles.php?tPath=' . $topics ['topics_id'] . '" target="_BLANK">Url To Category</a>';
			?>
...
<?php
		...
		while ( $articles = tep_db_fetch_array ( $articles_query ) ) {
			...
			'<a href="' . HTTP_SERVER . '' . DIR_WS_CATALOG . 'article_info.php?articles_id=' . $articles ['articles_id'] . '" target="_BLANK">Url To Article</a>';
			?>
