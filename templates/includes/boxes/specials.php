<?php
  ...
  if ($customer_group_id == '0') {
      $random_product_arr = tepps_special_product("select p.products_id,p.map_price, p.msrp_price,pd.products_short, pd.products_name, p.products_price, p.products_tax_class_id, p.products_image,pd.products_url, s.specials_new_products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SPECIALS . " s where p.products_status = '1' and p.products_id = s.products_id and pd.products_id = s.products_id and pd.language_id = '" . (int)$languages_id . "' and s.status = '1' order by s.specials_date_added desc limit " . PWNO_OF_SPECIAL_DISPLAY);
  } else {
      $random_product_arr = tepps_special_product("select p.products_id,pd.products_url,p.map_price, p.msrp_price, pd.products_name,pd.products_short, IF(pg.customers_group_price IS NOT NULL,pg.customers_group_price, p.products_price) as products_price, p.products_tax_class_id, p.products_image, s.specials_new_products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SPECIALS . " s LEFT JOIN " . TABLE_PRODUCTS_GROUPS . " pg using (products_id, customers_group_id) where p.products_status = '1' and p.products_id = s.products_id and pd.products_id = s.products_id and pd.language_id = '" . (int)$languages_id . "' and s.status = '1' and s.customers_group_id= '" . $customer_group_id . "' order by s.specials_date_added desc limit " . PWNO_OF_SPECIAL_DISPLAY);
  }
