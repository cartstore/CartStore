      ...
      if ($categories['parent_id'] == 0) {
          ...
          while ($subcategories = tep_db_fetch_array($subcategories_query)) {
              ...
              if ($subcategories['parent_id'] == $categories['categories_id'] && $categories['categories_id'] == $top_level) {
                  ...
                  $text_subcategories .= '<li><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_new_sub, 'NONSSL') . '">' . $subcategories['categories_name'] . '</a></li>' . " ";
              }
              
              } 
              $info_box_contents[$row] = array('align' => 'left', 'params' => '', 'text' => '<a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_new, 'NONSSL') . '" class="categories_top"><b>' . $categories['categories_name'] . '</b></a>' . $text_subcategories);
              $col++;
              if ($col > 0) {
                  $col = 0;
                  $row++;
