 ...
 */
 function add_column_if_not_exist($dbtable, $column, $column_attr = "varchar( 255 ) NULL" ) {
     $exists = false;
     $columns = tep_db_query("show columns from $dbtable");
     while ($c =tep_db_fetch_array($columns)) {
       if ($c['Field'] == $column){
         $exists = true;
         break;
       }
     ...
     if (!$exists) {
       tep_db_query("ALTER TABLE `$dbtable` ADD `$column`  $column_attr");
     }
   }
 ...
 */
  ...
  $sts->template['sociallogin_popup'] = '';
    ...
    if (empty($lrdata['Email']) && $emailrequired == 'True') {
      ...
      if (!empty($check_customer) && $check_customer > 0) {
	    ...
	    foreach($lrdata as $key => $value) {
	      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description) values ('Store tmp data', '".$lrdata['session']."', '".tep_db_input($value)."', '".tep_db_input($key)."')");
	    }
		$msg = "Please enter email to proceed.";
        $sts->template['sociallogin_popup'] = sociallogin_popup($msg, $lrdata);
      } 
    }
  }// Obj checking ends.
...
// Checking for popup button click.
  if (isset($_POST['LoginRadiusEmailClick']) && !empty($_POST['session'])) {
     $lrdata['session'] = tep_db_input($_POST['session']);
     if (tep_validate_email($_POST['email']) != true) {
       ...
       print sociallogin_popup($msg ,$lrdata);
		...
		$check_existEmail = tep_db_query("select customers_id, customers_firstname, customers_password, customers_email_address, customers_default_address_id from " . TABLE_CUSTOMERS . " where customers_email_address = '" . tep_db_input($_POST['email']) . "'");
	     ...
	     if($check_customer > 0) {
			 ...
			 $query = tep_db_query("select configuration_title, configuration_key, configuration_value, configuration_description from " . TABLE_CONFIGURATION . " where configuration_key = '" . tep_db_input($_POST['session']) . "'");
			  while($tmp_data = tep_db_fetch_array($query)) {
				$key = $tmp_data['configuration_description'];
				$value = $tmp_data['configuration_value'];
				$lrdata[$key] = $value;
	          }
             $lrdata['Email'] = tep_db_input($_POST['email']);
			 sociallogin_add_newUser($lrdata);
	     }
      }
   }
