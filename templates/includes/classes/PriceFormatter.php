...
$product_info['specials_new_products_price'] = $specials_price['specials_new_products_price'];
  ...
  function getPriceString($style='productPriceInBox') {
    ...
    /*	$lc_text = '<table align="top" border="1" cellspacing="0" cellpadding="0">';
        $lc_text .= '<tr><td align="center" class=' . $style. ' coldiv="2">';
	      $lc_text .= '&nbsp;<s>'
		. $currencies->display_price($this->thePrice,
				     tep_get_tax_rate($this->taxClass))
		. '</s>&nbsp;&nbsp;<div class="productSpecialPrice">'
	...
	<p class="lead text-info lead no-margin" itemprop="offerDetails" itemscope itemtype="http://data-vocabulary.org/Offer">
	<meta itemprop="currency" content="USD" />'
		. '<span itemprop="price">' . $currencies->display_price($this->specialPrice,
				     tep_get_tax_rate($this->taxClass))
		. '
</span></p>
...
<s>'. $currencies->display_price($this->thePrice,
        ...
        $lc_text .= ' <li class="save">Save <span>'
    ...
    {
		$lc_text = '<span itemprop="offerDetails" itemscope itemtype="http://data-vocabulary.org/Offer">
	<meta itemprop="currency" content="USD" /><span itemprop="price">'
		. $currencies->display_price($this->thePrice,
		tep_get_tax_rate($this->taxClass))
		. '</span></span>';
    }
	...
	if ($msrp['msrp_price'] != "0.00")
		$lc_text .= '<s class="text-muted"> Msrp ' . $currencies->display_price($msrp['msrp_price'], tep_get_tax_rate($this->taxClass)) . '</s></b></p>';
	...
	$lc_text .= '<p class="text-success quantity-price">Quantity Pricing
';
		for($i=1; $i<=8; $i++) {
			if($this->quantity[$i] > 0) {
				$lc_text .= ''
		...
		if ($this->hasSpecialPrice == true) {
			$lc_text = '<p class="text-info lead no-margin" itemprop="offerDetails" itemscope itemtype="http://data-vocabulary.org/Offer">
	<meta itemprop="currency" content="USD" /><span itemprop="price">
			'
  ...
  function getPriceStringShort() {
    global $currencies, $currency;
    if ($this->hasSpecialPrice == true) {
      $lc_text = '<span itemprop="offerDetails" itemscope itemtype="http://data-vocabulary.org/Offer-aggregate">
	<meta itemprop="currency" content="' . $currency . '"><s class="text-muted">'
	. $currencies->display_price($this->thePrice,
				     tep_get_tax_rate($this->taxClass))
	. '</s> <span itemprop="price">'
	. $currencies->display_price($this->specialPrice,
				     tep_get_tax_rate($this->taxClass))
	. '</span> ';
    }
	...
	$lc_text = '&nbsp;<span itemprop="offerDetails" itemscope itemtype="http://data-vocabulary.org/Offer">
	<meta itemprop="currency" content="' . $currency . '"><span itemprop="price">'
	  . $currencies->display_price($this->thePrice,
				       tep_get_tax_rate($this->taxClass))
	  . '</span></span>';
      }
  ...
  function getQtyPriceTable($style='productPriceInBox') {
    global $currencies;
      // If you want to change the format of the price/quantity table
      // displayed, here is where you do it.
    if($this->hasQuantityPrice == true) {
	    $lc_text .= ''."\n";
	    $lc_text .= '<ul class="list-group grid-view-none">'."\n";
		for($i=1; $i<=8; $i++) {
			if($this->quantity[$i] > 0) {
				$lc_text .= ' <li class="list-group-item">' .	 $this->quantity[$i] .'+&nbsp;</span><span class="badge">' . $currencies->display_price($this->price[$i],tep_get_tax_rate($this->taxClass))
				.'</span></li>'."\n";
			}
		}
       $lc_text .= "</ul>\n\n";
