...
//		NJW MOD - Repurpose target for ue as "selected" class marker (templates modified to include target in class declaration
		$this->tree[$cnt]['parsed_target'] = ($this->tree[$cnt]['target'] == '') ? '' : '' . $this->tree[$cnt]['target'];
...
/**
* A method to select the current item of $menu_name specifying a regular expression that matches (a substring of) the current URL; just the same as the setSelectedItemByUrl() method, but using preg_match(//i) instead of strpos()
* @access public
* @param string $menu_name the name of the menu for which the current item
*   has to be selected
* @param string $url_preg_matchi the regular expression that matches
