	...
	)
{
	...
	for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {
		...
		if ($this->tree[$cnt]['parsed_href'] == '' || $this->tree[$cnt]['parsed_href'] == '#') {
			$a_href_open_img = '';
			$a_href_close_img = '';
	    // NJW MOD to make tree node links clickable
      if ($GLOBALS['dm_tree_titleclick'])
			    $a_href_open = "<a onmousedown=\"". $toggle_function_name . "('" . $cnt . "')\" class=\"phplmnormal\">";
			else
			    $a_href_open = '<a class="phplmnormal">';
			// END NJW MOD						
			$a_href_close = '</a>';
		} else {
			// NJW MOD - Target for tree menus to repurpose as selected class
			$a_href_open_img = '<a href="' . $this->tree[$cnt]['parsed_href'] . '"' . $this->tree[$cnt]['parsed_title'] . '>';
			$a_href_close_img = '</a>';
			$a_href_open = '<a href="' . $this->tree[$cnt]['parsed_href'] . '"' . $this->tree[$cnt]['parsed_title'] . ' class="' . $linkstyle . ' '. $this->tree[$cnt]['parsed_target']. '">';
			$a_href_close = '</a>';
		}
		
		// NJW MOD - No tree image option
		if ($GLOBALS['dm_tree_folder_icons']) {		

		    if ($not_a_leaf) {
			    // NJW MOD to make tree node links clickable
					if ($GLOBALS['dm_tree_titleclick'])
					    $this->_treeMenu[$menu_name] .= "<a onmousedown=\"". $toggle_function_name . "('" . $cnt . "')\" onmouseover=\"style.cursor='hand'; style.cursor='pointer'\"><img align=\"top\" border=\"0\" class=\"imgs\" id=\"jt" . $cnt . "folder\" src=\"" . $img_folder_open . "\" alt=\"" . $alt_folder_open . "\" />" . $a_href_close_img;
					else
		   ...
		   $this->_treeMenu[$menu_name] .= "<SPAN id=\"jt" . $cnt . "folder\"></SPAN>";
