...
function tep_image($src, $alt = '', $width = '', $height = '', $params = '') {
  ...
  if ($image_size) {
      ...
      //Encrypt the image filename if switched on
       ...
       //Return the html
        $image = '<img class="thumbnail img-responsive" src="imagemagic.php?img='.$src.'&amp;w='.
...
// Outputs a button in the selected language
  function tep_image_submit($image, $alt = '', $parameters = '') {
    global $language;
$image_submit = '<input class="btn btn-info" type="submit" value="' . tep_output_string($alt) . '"';
 ...
 function tep_image_submit2($image, $alt = '', $parameters = '') {
    global $language;
$image_submit = '<input  class="btn btn-primary" type="image" src="' . tep_output_string(DIR_WS_LANGUAGES . $language . '/images/buttons/' . $image) . '" value="' . tep_output_string($alt) . '"';
    if (tep_not_null($alt)) $image_submit .= ' title=" ' . tep_output_string($alt) . ' "';
    if (tep_not_null($parameters)) $image_submit .= ' ' . $parameters;
    $image_submit .= ' />';
    return $image_submit;
...
// Output a separator either through whitespace, or with an image
  function tep_draw_separator($image = 'pixel_black.gif', $width = '100%', $height = '1') {
    return tep_image(DIR_WS_IMAGES . $image, '', $width, $height, 'style="height: ' . $height . 'px"');
  }
////
// Output a form
...
// Output a form input field
  function tep_draw_input_field($name, $value = '', $parameters = '', $type = 'text', $reinsert_value = true) {
	...
	if (preg_match('/class="/',$field)){
		$field = preg_replace('/class="/','class="form-control phoenix-input ',$field);
	} else {
		$field = str_replace('type="','class="form-control phoenix-input" type="',$field);
	}
    return $field;
  }
////
...
// Output a selection field - alias function for tep_draw_checkbox_field() and tep_draw_radio_field()
  function tep_draw_selection_field($name, $type, $value = '', $checked = false, $parameters = '') {
	...
	$field = $selection;
	if (preg_match('/class="/',$field)){
		$field = preg_replace('/class="/','class="" ',$field);
	} else {
		$field = str_replace('type="','class="" type="',$field);
	}
    return $field;
  }
////
...
// Output a form textarea field
  function tep_draw_textarea_field($name, $wrap, $width, $height, $text = '', $parameters = '', $reinsert_value = true) {
    $field = '<textarea class="form-control phoenix-input" name="' . tep_output_string($name) . '" wrap="' . tep_output_string($wrap) . '" cols="' . tep_output_string($width) . '" rows="' . tep_output_string($height) . '"';
    if (tep_not_null($parameters)) $field .= ' ' . $parameters;
    $field .= '>';
    if ( (isset($GLOBALS[$name])) && ($reinsert_value == true) ) {
...
// Output a form pull down menu
  function tep_draw_pull_down_menu($name, $values, $default = '', $parameters = '', $required = false) {
    $field = '<select class="form-control" id="' . tep_output_string($name) . '" name="' . tep_output_string($name) . '"';
    if (tep_not_null($parameters)) $field .= ' ' . $parameters;
...
// Output a jQuery UI Button
  function tep_draw_button($title = null, $icon = null, $link = null, $priority = null, $params = null) {
    ...
    if ( ($params['type'] == 'button') && isset($link) ) {
      $button .= '<a class="btn btn-default btn-sm" id="tdb' . $button_counter . '" href="' . $link . '"';

      if ( isset($params['newwindow']) ) {
        $button .= ' target="_blank"';
      }
    } else {
      $button .= '<button class="btn btn-default id="tdb' . $button_counter . '" type="' . tep_output_string($params['type']) . '"';
    }
