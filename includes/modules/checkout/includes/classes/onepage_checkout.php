...
class osC_onePageCheckout
	...
	function buildSession($forceReset = false)
		...
		if (!tep_session_is_registered('onepage') || $forceReset === true)
			...
			'customer'       => array(
			'firstname' => '',  'lastname' => '', 'company' => '',  'street_address' => '', 'street_address_2' => '',
	...
	function setPaymentMethod($method){
		...
		if ($confirmation !== false){
			...
			if ($inputFields != ''){
				$inputFields = '<div class="paymentFields payment_module_outputXXX"><blockquote>' .
				$inputFields .
				'</blockquote></div>';
			}
		}
...
//				$max_points = calculate_max_points($customer_shopping_points);
	...
	function setCheckoutAddress($action){
		...
		$order->{$varName}['street_address_2'] = tep_db_prepare_input($_POST[$prefix . 'street_address_2']);
	...
	function setAddress($addressType, $addressID){
		...
		'company'   => $address['entry_company'],   'street_address' => $address['entry_street_address'], 'street_address_2' => $address['entry_street_address_2'],
	...
	function confirmCheckout(){
		...
		$hiddenFields .
		'<script>
					 document.write(\'<img src="' . DIR_WS_MODULES . 'checkout/images/ajax-loader.gif"><h1>Processing Order, Please Wait...</h1>\');
					 redirectForm.submit();
				   </script><button class="btn btn-link">Continue</button></form>';
...
//				$max_points = calculate_max_points($customer_shopping_points);
				...
				foreach($_POST as $varName => $val){
					if (is_array($_POST[$varName])){
						foreach($_POST[$varName] as $varName2 => $val2){
							if (is_array($val2)){
								foreach ($val2 as $varName3 => $val3){
									$hiddenFields .= tep_draw_hidden_field($varName . "[" . $varName2 . "][" . $varName3 . "]", $val3);
								}
							} else {
						...
						if ($paymentMethod == 'paypal_standard' && $varName == 'shipping'){
						  // do nothing, "shipping" is a reserved Paypal field and messes up
						} else {
		...
		'<script>
           	...
           	</script><button class="btn btn-link">Here</button></form>';
	...
	function createCustomerAccount(){
		...
		if ($onepage['createAccount'] === true && $this->checkEmailAddress($onepage['customer']['email_address'])){
			...
			'entry_street_address_2' => $onepage['billing']['street_address_2'],
			...
			if (isset($_POST['diffShipping'])){
				...
				'entry_street_address_2' => $onepage['delivery']['street_address_2'],
	...
	function setDefaultSendTo(){
		...
		if (!tep_session_is_registered('sendto') || ($sendto < 1 && $customer_default_address_id > 0)) {
			$sendto = $customer_default_address_id;
			tep_session_register('sendto');
		} else {
	...
	function setDefaultBillTo(){
		...
		if (!tep_session_is_registered('billto') || ($billto < 1 && $customer_default_address_id > 0)) {
			$billto = $customer_default_address_id;
			tep_session_register('billto');
		} else {
