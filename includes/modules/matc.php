...
?>
<p><h5><?php echo MATC_HEADING_CONDITIONS; ?></h5>
 <div class="scroll_terms"><small><?php echo $textarea_contents; ?></small></div></p>
...
<div id="CAparagraph"><p>
		<?php
			if(MATC_SHOW_LINK != 'false'){
				echo '<div class="checkbox"><label>' .sprintf(MATC_CONDITION_AGREEMENT, tep_href_link(MATC_FILENAME, MATC_PARAMETERS)) .'</label>';
			}else{
				echo strip_tags(MATC_CONDITION_AGREEMENT);
			}

			echo '' .tep_draw_checkbox_field('TermsAgree','true', false, 'id="TermsAgree" onchange="updatebutton()"') . '</div>';
		?></p>
	</div>
