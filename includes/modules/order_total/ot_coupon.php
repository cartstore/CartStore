...
global $PHP_SELF, $order, $currencies;
  ...
  if ($od_amount > 0) {
    ...
    $this->output[] = array('title' => $this->title . ': ' . $this->coupon_code .':',
                            'text' => '<b><font color=red>(' . $currencies->format($od_amount) . ')</b>',
                            'value' => $od_amount);
  }
}
...
function credit_selection() {
  ...
  $selection_string .= '<div class="form-group">' . tep_draw_input_field('gv_redeem_code');
  $selection_string .= tep_draw_hidden_field('submit_redeem_coupon_x', 'true');
  $selection_string .= '<br>' . tep_image_submit('button_redeem.gif', IMAGE_REDEEM_VOUCHER, 'onclick="return submitFunction()"');
  $selection_string .= '</div></form>';
    ...
    if (tep_db_num_rows($coupon_query) !=0 ) {
      ...
      if ($get_result['coupon_minimum_order'] <= $this->get_order_total()) {
        if ($get_result['restrict_to_products'] || $get_result['restrict_to_categories']) {
          ...
          } else { //Not restricted to a product or category
            if ($get_result['coupon_type'] !='P') {
              $od_amount = $c_deduct;
            } else {
...
function install() {
  ...
  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_COUPON_SORT_ORDER', '5', 'Sort order of display.', '6', '2', now())");
