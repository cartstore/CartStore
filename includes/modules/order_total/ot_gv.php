<?php
  class ot_gv
  {
      ...
      function mod_process()
      {
          ...
          if ($my_od_amount > 0) {
              $this->my_output[] = array('title' => $this->title . ':', 'text' => '<b>' . $currencies->format($my_od_amount) . '</b>', 'value' => $my_od_amount);
          } //if ($my_od_amount > 0)
      } //function mod_process()
      ...
      function credit_selection()
      {
          ...
          if (tep_db_num_rows($gv_query)) {
              ...
              $selection_string .= '';
              $selection_string .= ' ' . "\n";
              $image_submit = '<input class="button" type="submit" name="submit_redeem" Value="Redeem" title = "' . IMAGE_REDEEM_VOUCHER . '">';
              $selection_string .= TEXT_ENTER_GV_CODE . tep_draw_input_field('gv_redeem_code') . '';
              $selection_string .= '' . $image_submit . '';
              $selection_string .= '' . "\n";
          } //if (tep_db_num_rows($gv_query))
      ...
      function collect_posts()
      {
          ...
          if ($_POST['submit_redeem_x'] && !$_POST['gv_redeem_code'])
      ...
      function calculate_tax_deduction($amount, $od_amount, $method)
      {
          ...
          {
              global $_GET;
              $error = array('title' => MODULE_ORDER_TOTAL_GV_TEXT_ERROR, 'error' => stripslashes(urldecode($_GET['error'])));
              return $error;
          } //function get_error()
