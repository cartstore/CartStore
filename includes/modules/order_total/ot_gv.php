       ...
       if (tep_session_is_registered('cot_gv')) {
        ...
        if ($od_amount > 0) {
          $this->output[] = array('title' => $this->title . ':',
                           'text' => '<b><font color=red>(' . $currencies->format($od_amount) . ')</font></b>',
                           'value' => $od_amount);
        }
      }
    }
  ...
  function update_credit_account($i, $order_id=0) {
    ...
    function credit_selection() {
      ...
      if (tep_db_num_rows($gv_query)) {
 	   	//BOE (fnzb) changed image to button using hidden field to pass submit_redeem_x 
		  ...
		  $selection_string .= tep_draw_hidden_field('submit_redeem_x', 'true');
		  $selection_string .= '<input type="image" name="submit_redeem" onClick="submitFunction()" src="' . DIR_WS_LANGUAGES . $language . '/images/buttons/button_redeem.gif" border="0" alt="' . IMAGE_REDEEM_VOUCHER . '" title = "' . IMAGE_REDEEM_VOUCHER . '">';
    ...
    function apply_credit() {
      ...
      if (tep_session_is_registered('cot_gv')) {
        $gv_query = tep_db_query("select amount,amount_redeemed from " . TABLE_COUPON_GV_CUSTOMER . " where customer_id = '" . $customer_id . "'");
        $gv_result = tep_db_fetch_array($gv_query);
        $gv_payment_amount = $this->deduction;
        $gv_amount = $gv_result['amount'] - $gv_payment_amount;
		$gv_amount_redeemed = $gv_result['amount_redeemed'] + $gv_payment_amount;
        $gv_update = tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . $gv_amount . "',amount_redeemed = '" . $gv_amount_redeemed . "' where customer_id = '" . $customer_id . "'");
      }
    ...
    function get_error() {
      ...
      $error = array('title' => MODULE_ORDER_TOTAL_GV_TEXT_ERROR,
                     'error' => stripslashes(urldecode(html_entity_decode($_GET['error']))));
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_GV_SORT_ORDER', '4', 'Sort order of display.', '6', '2', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Re-calculate Tax', 'MODULE_ORDER_TOTAL_GV_CALC_TAX', 'Credit Note', 'Re-Calculate Tax', '6', '7','tep_cfg_select_option(array(\'None\', \'Standard\', \'Credit Note\'), ', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function ,date_added) values ('Credit including Tax', 'MODULE_ORDER_TOTAL_GV_CREDIT_TAX', 'true', 'Add tax to purchased Gift Voucher when crediting to Account', '6', '8','tep_cfg_select_option(array(\'true\', \'false\'), ', now())");
    }
