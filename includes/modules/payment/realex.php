...
// class methods
    ...
    function pre_confirmation_check() {
      ...
      $result = $cc_validation->validate($_POST['realexpay_cc_number'], $_POST['realexpay_cc_expires_month'], $_POST['realexpay_cc_expires_year'], $_POST['cvn_number']);
      ...
      if ( ($result == false) || ($result < 1) ) {
        $payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode($error) . '&realexpay_cc_owner=' . urlencode($_POST['realexpay_cc_owner']) . '&realexpay_cc_type=' . urlencode($_POST['realexpay_cc_type']) . '&realexpay_cc_expires_month=' . $_POST['realexpay_cc_expires_month'] . '&realexpay_cc_expires_year=' . $_POST['realexpay_cc_expires_year'];

        tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
      }

      $this->cc_card_owner = $_POST['realexpay_cc_owner']; 
      $this->cc_card_number = $cc_validation->cc_number;
      $this->cc_expiry_month = $cc_validation->cc_expiry_month;
      $this->cc_expiry_year = $cc_validation->cc_expiry_year;
      $this->cc_card_type = $cc_validation->cc_type;
	  $this->cvn_number = $_POST['cvn_number'];
    ...
    if (ONEPAGE_CHECKOUT_ENABLED == 'True' && SELECT_VENDOR_SHIPPING != 'true'){
      ...
      $confirmation = array('title' => $this->title,
                           'fields' => array(array('title' => MODULE_PAYMENT_REALEX_TEXT_CREDIT_CARD_OWNER,
                                                   'field' => $_POST['realexpay_cc_owner']),
                       array('title' => MODULE_PAYMENT_REALEX_TEXT_CREDIT_CARD_TYPE,
                                             ...
                                             array('title' => 'CVN Number', 'field' =>  $this->cvn_number),                                           array('title' => MODULE_PAYMENT_REALEX_TEXT_CREDIT_CARD_EXPIRES,
                                                   'field' => strftime('%B, %Y', mktime(0,0,0,$_POST['realexpay_cc_expires_month'], 1, '20' . $_POST['realexpay_cc_expires_year']))))); 
...
$amount = $_POST['amount'];
$ccnum =$_POST['creditCard1'];
$ccname = $_POST['ccowner'];

$ccctype = $_POST['currency'];
$amount2 = $_POST['amount2'];

$curr = $_POST['currency'];
$expdate = $_POST['exdate1'] . $_POST['exdate2'];
