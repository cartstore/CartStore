...
// class methods
    ...
    function pre_confirmation_check() {
      ...
      $result = $cc_validation->validate($_POST['payquake_cc_number'], $_POST['payquake_cc_expires_month'], $_POST['payquake_cc_expires_year']);
      $error = '';
      switch ($result) {
        case -1:
      ...
      if ( ($result == false) || ($result < 1) ) {
        $payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode($error) . '&payquake_cc_owner=' . urlencode($_POST['payquake_cc_owner']) . '&payquake_cc_expires_month=' . $_POST['payquake_cc_expires_month'] . '&payquake_cc_expires_year=' . $_POST['payquake_cc_expires_year'] . (MODULE_PAYMENT_PAYQUAKE_CC_VERIFY_WITH_CVC == 'True' ? '&payquake_cc_cvc=' . (int)$_POST['payquake_cc_cvc'] : '');
    ...
    function confirmation() {
      ...
      if (MODULE_PAYMENT_PAYQUAKE_CC_VERIFY_WITH_CVC == 'True') {
        $confirmation['fields'][] = array('title' => MODULE_PAYMENT_PAYQUAKE_CC_CREDIT_CARD_CVC,
                                          'field' => (int)$_POST['payquake_cc_cvc']);
      }
    ...
    function process_button() {
      ...
      $process_button_string = tep_draw_hidden_field('payquake_cc_owner', $_POST['payquake_cc_owner']) .
      ...
      if (MODULE_PAYMENT_PAYQUAKE_CC_VERIFY_WITH_CVC == 'True') {
        $process_button_string .= tep_draw_hidden_field('payquake_cc_cvc', (int)$_POST['payquake_cc_cvc']);
      }
    ...
    function before_process() {
      ...
      $params = array('action' => 'ns_quicksale_cc',
                      ...
                      'ccname' => $_POST['payquake_cc_owner'],
      ...
      if (MODULE_PAYMENT_PAYQUAKE_CC_VERIFY_WITH_CVC == 'True') {
        $params['cvv2'] = (int)$_POST['payquake_cc_cvc'];
      }
      ...
      if ($error) {
        tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . urlencode($error) . '&payquake_cc_owner=' . urlencode($_POST['payquake_cc_owner']) . '&payquake_cc_expires_month=' . $this->cc_expiry_month . '&payquake_cc_expires_year=' . $this->cc_expiry_year . (MODULE_PAYMENT_PAYQUAKE_CC_VERIFY_WITH_CVC == 'True' ? '&payquake_cc_cvc=' . (int)$_POST['payquake_cc_cvc'] : ''), 'SSL'));
      }
    }
