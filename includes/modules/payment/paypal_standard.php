...
// class methods
    ...
    function selection() {
      global $cart_PayPal_Standard_ID, $order;
...
//                          'amount' => $this->format_raw($order->info['total'] - $order->info['shipping_cost'] - $order->info['tax']),
      ...
      for ($i = 0; $i < count($order->products); $i++){
       $parameters['item_name_'.($i+1)] = $order->products[$i]['name'];
       $parameters['quantity_'.($i+1)]  = $order->products[$i]['qty'];
       $parameters['amount_'.($i+1)]    = $order->products[$i]['final_price'];
       $parameters['tax_rate_'.($i+1)]  = (float)$order->products[$i]['tax'];
       $sum += $order->products[$i]['final_price'] * $order->products[$i]['qty'];
      }
      if ($sum > $this->format_raw($order->info['total'] - $order->info['shipping_cost'] - $order->info['tax'])){
        $parameters['discount_amount_1'] = (float)($sum - $this->format_raw($order->info['total'] - $order->info['shipping_cost'] - $order->info['tax']));
      }
      ...
      if (is_numeric($sendto) && ($sendto > 0)) {
		...
		if (!empty($order->delivery['street_address_2']))
        	$parameters['address2'] = $order->delivery['street_address_2'];
		...
		if (!empty($order->billing['street_address_2']))
        	$parameters['address2'] = $order->billing['street_address_2'];
...
// Start - CREDIT CLASS Gift Voucher Contribution
   ...
   if (isset($order_total_modules))    $order_total_modules->apply_credit();
// End - CREDIT CLASS Gift Voucher Contribution
...
// Start - CREDIT CLASS Gift Voucher Contribution
   if (isset($order_total_modules))    $order_total_modules->clear_posts();
// End - CREDIT CLASS Gift Voucher Contribution
      tep_session_unregister('cart_PayPal_Standard_ID');
