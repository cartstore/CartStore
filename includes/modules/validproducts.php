...
<?php
if(!empty($search))
{   $result = tep_db_query("SELECT * FROM products p, products_description pd WHERE pd.products_name like '%" . tep_db_input($search) . "%' and p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' ORDER BY pd.products_name");
