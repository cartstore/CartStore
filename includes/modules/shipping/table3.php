...
// class constructor
    function table3() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_TABLE3_ZONE > 0) ) {
        ...
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_TABLE3_ZONE . "' and zone_country_id = '" . $zoneRow['entry_country_id'] . "' order by zone_id");
...
$disallowed_postcodes= split("[,]", MODULE_SHIPPING_TABLE3_DISALLOWED); 
...
////			
      ...
      $table3_cost = split("[:,]" , MODULE_SHIPPING_TABLE3_COST);
      $size = sizeof($table3_cost);
      for ($i=0, $n=$size; $i<$n; $i+=2) {
        if ($order_total <= $table3_cost[$i]) {
          $shipping = $table3_cost[$i+1];
          break;
        }
      }

			if ($shipping==''){
			$shipping = $table3_cost[$size-2];
			}
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Table Method', 'MODULE_SHIPPING_TABLE3_STATUS', 'True', 'Do you want to offer table3 rate shipping?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
	  ...
	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_TABLE3_SORT_ORDER', '0', 'Sort order of display.', '6', '3', now())");
	  ...
	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Table 3 Title', 'MODULE_SHIPPING_TABLE3_TEXT_TITLE', 'Table 3 Title', 'The text used as the title of this module', '6', '5', now())");
	  ...
	  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Handling Fee', 'MODULE_SHIPPING_TABLE3_HANDLING', '0', 'Handling fee for this shipping method.', '6', '7', now())");
    ...
    function keys() {
      return array('MODULE_SHIPPING_TABLE3_STATUS', 'MODULE_SHIPPING_TABLE3_TAX_CLASS', 'MODULE_SHIPPING_TABLE3_SORT_ORDER',  'MODULE_SHIPPING_TABLE3_ZONE', 'MODULE_SHIPPING_TABLE3_TEXT_TITLE', 'MODULE_SHIPPING_TABLE3_COST', 'MODULE_SHIPPING_TABLE3_HANDLING', 'MODULE_SHIPPING_TABLE3_MODE','MODULE_SHIPPING_TABLE3_DISALLOWED' );
    }
  }
?>