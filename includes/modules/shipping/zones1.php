...
// class constructor
   function zones1() {
     ...
     $this->num_zones1 = ZONES_MODULE_NUMBER;
   }
...
// class methods
   function quote($method = '') {
     ...
     for ($i=1; $i<=$this->num_zones1; $i++) {
       ...
       $country_zones1 = preg_split("/[,]/", $countries_table);
...
//ECHO $dest_country.'-dfgfdg';exit;
     ...
     if ($dest_zone == 0) {
       $dest_zone = $this->num_zones1;    // the zone is the lastest zone avalaible
     ...
     if ($dest_zone == 0) {
       ...
       $zones1_table = preg_split("/[:,]/" , $zones1_cost);
       $size = sizeof($zones1_table);
       for ($i=0; $i<$size; $i+=2) {
         if ($shipping_weight <= $zones1_table[$i]) {
           $shipping = $zones1_table[$i+1];
           $shipping_method = MODULE_SHIPPING_ZONES1_TEXT_WAY . ' ' . $order->delivery['country']['title'] . ': ';
           if ($shipping_num_boxes > 1) {
             $shipping_method .= $shipping_num_boxes . 'x ';
           }
           $shipping_method .= $shipping_weight . ' ' . MODULE_SHIPPING_ZONES1_TEXT_UNITS;
           break;
         }
       }
       ...
       if ($shipping == -1) {
         ...
         $shipping_cost = ($shipping * $shipping_num_boxes) + constant('MODULE_SHIPPING_ZONES1_HANDLING_' . $dest_zone);
       }
     }
   ...
   // elari - Added to select default country if not in listing
   function install() {
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_ZONES1_STATUS', 'True', 'Do you want to offer zone rate shipping?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
     ...
     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_SHIPPING_ZONES1_SORT_ORDER', '0', 'Sort order of display.', '6', '0', now())");
      ...
      $default_countries = 'AT,BE,DE,FR,GL,IS,IE,IT,NL,NO,DK,PL,ES,SE,CH,FI,PT,IL,GR,';
         $shipping_table = '1.1:2.99,1.5:4.99,1.9:10.99,20:14.99,';
       }
       if ($i == 2) {
         $default_countries = 'CA,US,';
         $shipping_table = '0.5:2.99,1.0:4.99,';
       }
       if ($i == $this->num_zones1) {
       ...
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Shipping Table', 'MODULE_SHIPPING_ZONES1_COST_" . $i ."', '" . $shipping_table . "', 'Shipping rates to Zone " . $i . " destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone " . $i . " destinations.', '6', '0', now())");
       tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone " . $i ." Handling Fee', 'MODULE_SHIPPING_ZONES1_HANDLING_" . $i ."', '0', 'Handling Fee for this shipping zone', '6', '0', now())");
			}
			 tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Zone 1 Title', 'MODULE_SHIPPING_ZONES1_TEXT_TITLE', 'Zone 1 Title', 'The text used as the title of this module', '7', '0', now())");
			 tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Invalid Country Text', 'MODULE_SHIPPING_ZONES1_INVALID_ZONE', 'Sorry, this method is not available for your country.', 'The text used to inform customer that this method is not available outside the UK', '7', '0', now())");
			 tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Underfined Rate Text', 'MODULE_SHIPPING_ZONES1_UNDEFINED_RATE', 'The shipping rate cannot be determined at this time.', 'The text used if weight is not covered', '7', '0', now())");
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Order min weight', 'MODULE_SHIPPING_ZONES1_MIN_WEIGHT', '2000', 'Minimum weight in g(s) for order', '6', '7', now())");
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Order max weight', 'MODULE_SHIPPING_ZONES1_MAX_WEIGHT', '2000000000', 'Maximum weight in g(s) for order', '6', '8', now())");
   ...
   function keys() {
     $keys = array('MODULE_SHIPPING_ZONES1_STATUS', 'MODULE_SHIPPING_ZONES1_TAX_CLASS', 'MODULE_SHIPPING_ZONES1_SORT_ORDER', 'MODULE_SHIPPING_ZONES1_TEXT_TITLE', 'MODULE_SHIPPING_ZONES1_INVALID_ZONE', 'MODULE_SHIPPING_ZONES1_UNDEFINED_RATE','MODULE_SHIPPING_ZONES1_MIN_WEIGHT','MODULE_SHIPPING_ZONES1_MAX_WEIGHT');
