...
class usps
{
	...
	$title = str_replace(array('&lt;sup&gt;','&lt;/sup&gt;'),'', $title);
	...
	if ($order->delivery['country']['id'] == SHIPPING_ORIGIN_COUNTRY)
		...
		$body = preg_replace('/\&lt;sup\&gt;\&#174;\&lt;\/sup\&gt;/', ' regimark', $body);
		$body = preg_replace('/\&lt;sup\&gt;\&#8482;\&lt;\/sup\&gt;/', ' tradmrk', $body);
		...
		$service = preg_replace('/<sup>\&#174;<\/sup>/', ' regimark', $service);
		$service = preg_replace('/<sup>\&#8482;<\/sup>/', ' tradmrk', $service);
		$service = str_replace(array(' 1-Day', ' 2-Day'), '', $service);
			switch ($service)
			{

				case 'First-Class Mail regimark':
				case 'First-Class Mail tradmrk':
				...
				case 'First-Class Mail tradmrk Letter':
					$time = preg_match('/<Days>(.*)<\/Days>/', $transresp[$service], $tregs);
					$time = $tregs[1];
					if ($time == 'Military Mail') {$time = $time;}
					else {if ($this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($time + $this->processing);}
				...
				case 'First-Class Mail tradmrk Large Envelope':
					$time = preg_match('/<Days>(.*)<\/Days>/', $transresp[$service], $tregs);
					$time = $tregs[1];
					if ($time == 'Military Mail') {$time = $time;}
					else {if ($this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($time + $this->processing);}
				...
				case 'First-Class Mail tradmrk Package':
					$time = preg_match('/<Days>(.*)<\/Days>/', $transresp[$service], $tregs);
					$time = $tregs[1];
					if ($time == 'Military Mail') {$time = $time;}
					else {if ($this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($time + $this->processing);}
				...
				case 'Priority Mail tradmrk Flat Rate Envelope':
					$time = preg_match('/<Days>(.*)<\/Days>/', $transresp[$service], $tregs);
					$time = $tregs[1];
					if ($time == 'Military Mail') {$time = $time;}
					else {if ($this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($time + $this->processing);}
				...
				case 'Priority Mail tradmrk Small Flat Rate Box':
					$time = preg_match('/<Days>(.*)<\/Days>/', $transresp[$service], $tregs);
					$time = $tregs[1];
					if ($time == 'Military Mail') {$time = $time;}
					else {if ($this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($time + $this->processing);}
				...
				case 'Priority Mail tradmrk Medium Flat Rate Box':
					$time = preg_match('/<Days>(.*)<\/Days>/', $transresp[$service], $tregs);
					$time = $tregs[1];
					if ($time == 'Military Mail') {$time = $time;}
					else {if ($this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($time + $this->processing);}
				...
				case 'Priority Mail tradmrk Large Flat Rate Box':
					$time = preg_match('/<Days>(.*)<\/Days>/', $transresp[$service], $tregs);
					$time = $tregs[1];
					if ($time == 'Military Mail') {$time = $time;}
					else {if ($this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($time + $this->processing);}
					...
					break;
				case 'Priority Mail Express regimark':
				case 'Priority Mail Express tradmrk':
					...
					break;
				case 'Priority Mail Express regimark Flat Rate Envelope':
				case 'Priority Mail Express tradmrk Flat Rate Envelope':
					$time = preg_match('/<CommitmentDate>(.*)<\/CommitmentDate>/', $response[$i], $regs);
					$time = $regs[1];
					if ($time == 'Overnight to many areas' && $this->processing > 0) {$time = MODULE_SHIPPING_USPS_TEXT_ESTIMATED . ($this->processing + 1) . MODULE_SHIPPING_USPS_TEXT_DAYS;;}
					elseif ($time == 'Overnight to many areas') {$time = '---' . $time;}
