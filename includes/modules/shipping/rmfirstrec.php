...
class rmfirstrec {
	...
	function rmfirstrec() {
		...
		if ($this->weight_split) {
			if (SHIPPING_MAX_WEIGHT > MODULE_SHIPPING_RMFIRSTREC_MAX_WEIGHT) {
				$my_max_shipping_weight = MODULE_SHIPPING_RMFIRSTREC_MAX_WEIGHT;
			}
		}
		...
		if ($this->value_split) {
			...
			if ( ($order->info['subtotal'] / $shipping_num_boxes) > MODULE_SHIPPING_RMFIRSTREC_MAX_VALUE) {
				$my_shipping_num_boxes = ceil($order->info['subtotal']/MODULE_SHIPPING_RMFIRSTREC_MAX_VALUE);
		...
		if ( ($order->info['subtotal'] / $my_shipping_num_boxes) < MODULE_SHIPPING_RMFIRSTREC_MIN_VALUE) {
			$this->enabled = false;
		}

		// Only ship if packet value does not exceed method maximum value
		if ( ($order->info['subtotal'] / $my_shipping_num_boxes) > MODULE_SHIPPING_RMFIRSTREC_MAX_VALUE) {
			$this->enabled = false;
		}

		// Only ship if packet weight exceeds method minimum value
		if ( $my_shipping_weight < MODULE_SHIPPING_RMFIRSTREC_MIN_WEIGHT) {
			$this->enabled = false;
		}

		// Only ship if packet weight does not exceed method maximum value
		if ( $my_shipping_weight > MODULE_SHIPPING_RMFIRSTREC_MAX_WEIGHT) {
			$this->enabled = false;
		}
	...
	function quote($method = '') {
		...
		if (defined('MODULE_SHIPPING_RMFIRSTREC_COUNTRIES_' . $this->num_zones)) {
				...
				for ($i=0; $i<$size; $i+=2) {
					if ($my_shipping_weight <= $zones_table[$i]) {
						...
						if(tep_not_null($method) ) {
							...
							if (constant('MODULE_SHIPPING_RMFIRSTREC_DISPLAY_WEIGHT') == 'True') {
								// Delivery Weight : x items of n.nnnn Kg's
								$shipping_method = MODULE_SHIPPING_RMFIRSTREC_TEXT_WAY. ' : ';
								if ($my_shipping_num_boxes > 1) { // Shipment split between several boxes/packets
									$shipping_method = $shipping_method . $my_shipping_num_boxes . ' '.MODULE_SHIPPING_RMFIRSTREC_TEXT_ITEMS.' ';
								}else{ // Onlky 1 package in shipment
									$shipping_method = $shipping_method . $my_shipping_num_boxes . ' '.MODULE_SHIPPING_RMFIRSTREC_TEXT_ITEM.' ';
								}
								$shipping_method = $shipping_method . $my_shipping_weight. ' ' . MODULE_SHIPPING_RMFIRSTREC_TEXT_UNITS;
							}
							// Display insurance?
							if (constant('MODULE_SHIPPING_RMFIRSTREC_DISPLAY_INSURANCE') == 'True') {
								$shipping_method = $shipping_method . ' ('; // Open text with a bracket
							if ($insurance > 0) {
								// Insured upto £nnn.nn
								$shipping_method = $shipping_method . MODULE_SHIPPING_RMFIRSTREC_DELIVERY_INSURANCE . $insurance;
								}else{
									$shipping_method = $shipping_method . MODULE_SHIPPING_RMFIRSTREC_DELIVERY_NO_INSURANCE;
								}
								if (constant('MODULE_SHIPPING_RMFIRSTREC_DISPLAY_TIME') == 'True') {
							...
							if (constant('MODULE_SHIPPING_RMFIRSTREC_DISPLAY_TIME') == 'True') {
								...
								if (constant('MODULE_SHIPPING_RMFIRSTREC_DISPLAY_INSURANCE') == 'False') {
									//Insurance not diplayed so lead with a bracket and Uppercase first letter of shipping time
									$shipping_method = $shipping_method . ' (';
									$shipping_method = $shipping_method . ucfirst(MODULE_SHIPPING_RMFIRSTREC_DELIVERY_SHIPPING_TIME) . ')';
								}else{ // Insurance displayed so just add text of shipping time
									$shipping_method = $shipping_method . MODULE_SHIPPING_RMFIRSTREC_DELIVERY_SHIPPING_TIME . ')';
								}
							}
						}
						break;
			...
			// Shipping charge found, so add together all costs
				...
				$shipping_cost = (($shipping + $ic) * $my_shipping_num_boxes) + constant('MODULE_SHIPPING_RMFIRSTREC_HANDLING_' . $this->num_zones);
			}
		}
	...
	function install() {
		// MODULE_SHIPPING_RMFIRSTREC_VERSION
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Version', 'MODULE_SHIPPING_RMFIRSTREC_VERSION', '2.2.2', 'Sort order of display (1 shown first 99 etc shown last to customer)', '6', '0', now())");
		...
		// MODULE_SHIPPING_RMFIRSTREC_TAX_CLASS
		...
		// MODULE_SHIPPING_RMFIRSTREC_SORT_ORDER
		...
		// MODULE_SHIPPING_RMFIRSTREC_MIN_WEIGHT
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Minimum weight to ship', 'MODULE_SHIPPING_RMFIRSTREC_MIN_WEIGHT', '0', 'Enter the minimum weight to ship', '6', '0', now())");
		// MODULE_SHIPPING_RMFIRSTREC_MAX_WEIGHT
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Maximum weight to ship', 'MODULE_SHIPPING_RMFIRSTREC_MAX_WEIGHT', '10', 'Enter the maximum weight to ship', '6', '0', now())");
		// MODULE_SHIPPING_RMFIRSTREC_WEIGHT_SPLIT
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Split shipments on maximum weight to ship', 'MODULE_SHIPPING_RMFIRSTREC_WEIGHT_SPLIT', 'False', 'Do you want to split your shipment by maximum weight to ship?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
		// MODULE_SHIPPING_RMFIRSTREC_MIN_VALUE
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Minimum value to ship', 'MODULE_SHIPPING_RMFIRSTREC_MIN_VALUE', '0', 'Enter the minimum value to ship', '6', '0', now())");
		// MODULE_SHIPPING_RMFIRSTREC_MAX_VALUE
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Maximum value to ship', 'MODULE_SHIPPING_RMFIRSTREC_MAX_VALUE', '46', 'Enter the maximum value to ship in &pound;', '6', '0', now())");
		// MODULE_SHIPPING_RMFIRSTREC_VALUE_SPLIT
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Split shipments on maximum value to ship', 'MODULE_SHIPPING_RMFIRSTREC_VALUE_SPLIT', 'False', 'Do you want to split your shipment by maximum value to ship?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
		// MODULE_SHIPPING_RMFIRSTREC_DISPLAY_WEIGHT
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display delivery weight', 'MODULE_SHIPPING_RMFIRSTREC_DISPLAY_WEIGHT', 'True', 'Do you want to display the shipping weight? (e.g. Delivery Weight : 2.7674 Kg\'s)', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
		// MODULE_SHIPPING_RMFIRSTREC_DISPLAY_INSURANCE
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display Insurance', 'MODULE_SHIPPING_RMFIRSTREC_DISPLAY_INSURANCE', 'True', 'Do you want to display the shipping insurance? (e.g. Insured upto &pound;500)', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
		// MODULE_SHIPPING_RMFIRSTREC_DISPLAY_TIME
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display delivery time', 'MODULE_SHIPPING_RMFIRSTREC_DISPLAY_TIME', 'True', 'Do you want to display the shipping time? (e.g. Ships within 3 to 5 days)', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
		...
		for ($i = 1; $i <= $this->num_zones; $i++) {
			...
			if ($i == 1) {
				...
				$shipping_table = '.1:2.35,.25:2.73,.5:3.25,.75:3.82,1:4.48,1.25:5.67,1.5:6.43,1.75:7.19,2:7.95,4:9.72,6:12.77,8:15.82,10:18.87';
				$shipping_compensation = '46:0';
				$shipping_handling = 0;
			}
			// MODULE_SHIPPING_RMFIRSTREC_COUNTRIES_$i
			tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Valid ISO Country Codes', 'MODULE_SHIPPING_RMFIRSTREC_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Comma separated list of two character ISO country codes that are valid destinations for this method (Default: ".$default_countries.")', '6', '0', now())");
			// MODULE_SHIPPING_RMFIRSTREC_COST_$i
			tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('RM First Class Recorded Rates', 'MODULE_SHIPPING_RMFIRSTREC_COST_" . $i ."', '" . $shipping_table . "', 'Enter values upto 5,2 decimal places. (12345.67) Example: 2:4.2,4:6.85 - Weights less than or equal to 2Kg would cost 4.20, Weights less than or equal to 4Kg but more than 2Kg will cost 6.85. Do not enter KG or currency symbols.', '6', '0', now())");
			// MODULE_SHIPPING_RMFIRSTREC_COMPENSATION_$i
			tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('RM First Class Recorded Compensation Rates', 'MODULE_SHIPPING_RMFIRSTREC_COMPENSATION_" . $i ."', '" . $shipping_compensation . "', 'Enter values upto 5,2 decimal places. (12345.67) Example: 34:0,100:1,250:2.25 - Insurance cover for cart values upto 34 would cost 0.00 extra, those values more than 100 and upto 250 will cost 2.25 extra. Do not enter currency symbols.', '6', '0', now())");
			// MODULE_SHIPPING_RMFIRSTREC_HANDLING_$i
			tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Packaging / Handling Fee', 'MODULE_SHIPPING_RMFIRSTREC_HANDLING_" . $i ."', '" . $shipping_handling . "', 'If you want to add extra costs to customers for jiffy bags etc, the cost can be entered below (eg enter 1.50 for a value of 1.50)', '6', '0', now())");
		}
	}

	function remove() {
		tep_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key in ('MODULE_SHIPPING_RMFIRSTREC_VERSION','" . implode("', '", $this->keys()) . "')");
	}

	function keys() {
		$keys = array('MODULE_SHIPPING_RMFIRSTREC_STATUS'
				,'MODULE_SHIPPING_RMFIRSTREC_TAX_CLASS'
				,'MODULE_SHIPPING_RMFIRSTREC_SORT_ORDER'
				,'MODULE_SHIPPING_RMFIRSTREC_MIN_WEIGHT'
				,'MODULE_SHIPPING_RMFIRSTREC_MAX_WEIGHT'
		...
		for ($i=1; $i<=$this->num_zones; $i++) {
			...
			$keys[] = 'MODULE_SHIPPING_RMFIRSTREC_COMPENSATION_' . $i;
