...
<span class="ourprice_name">Our Price:</span> <span class="our_price_price"><a href="login.php">Login for Price</a></span>';
              ...
              if ($new_products[$i]['products_url'] != "") {
                  $newArea = '<div align="center"><span class="alternate_buy" ><a class="button" href=' . $new_products[$x]['products_url'] . '" title="' . $new_products[$x]['products_url'] . '" >Partner Buy </a></div>';
              } elseif (HIDE_PRICE_NON_LOGGED == "true" && $_SESSION['customers_email_address'] == '') {
                  $newArea = '';
              } elseif ($new_products[$x]['products_price'] > 0) {
                  $newArea = ' <a class="button" href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $new_products[$x]['products_id']) . '">Add to Cart</a>';
              } else {
              ...
              $info_box_contents[$row][$col] = array('align' => '', 'params' => '', 'text' => '<div class="productWrap">
                       ...
                       <h4><a title ="' . $new_products[$x]['products_short'] . '" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $new_products[$x]['products_id']) . '">' . $new_products[$x]['products_name'] . '</a></h4>
