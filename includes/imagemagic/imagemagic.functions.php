...
class imagemagic_functions {
	...
	function version_compare_replacement($version1, $version2, $operator='') {
		...
		for ($i = 0; $i < $parts_count; $i++) {
			$comparison = imagemagic_functions::version_compare_replacement_sub($version1, $version2, $operator);
			if ($comparison != 0) {
				return $comparison;
			}
		}
	...
	function ImagecolorAllocateAlphaSafe(&$gdimg_hexcolorallocate, $R, $G, $B, $alpha=false) {
		if (imagemagic_functions::version_compare_replacement(phpversion(), '4.3.2', '>=') && ($alpha !== false)) {
	...
	function ImageHexcolorAllocate(&$gdimg_hexcolorallocate, $HexColorString, $dieOnInvalid=false, $alpha=false) {
		...
		if (imagemagic_functions::IsHexColor($HexColorString)) {
			$R = hexdec(substr($HexColorString, 0, 2));
			$G = hexdec(substr($HexColorString, 2, 2));
			$B = hexdec(substr($HexColorString, 4, 2));
			return imagemagic_functions::ImageColorAllocateAlphaSafe($gdimg_hexcolorallocate, $R, $G, $B, $alpha);
		}
		if ($dieOnInvalid) {
	...
	function GrayscalePixel($OriginalPixel) {
		$gray = imagemagic_functions::GrayscaleValue($OriginalPixel['red'], $OriginalPixel['green'], $OriginalPixel['blue']);
	...
	function ImageResizeFunction(&$dst_im, &$src_im, $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH) {
		if (imagemagic_functions::gd_version() >= 2.0) {
	...
	function ImageCopyRespectAlpha(&$dst_im, &$src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $pct=100) {
		for ($x = $src_x; $x < $src_w; $x++) {
			for ($y = $src_y; $y < $src_h; $y++) {
				$RealPixel    = imagemagic_functions::GetPixelColor($dst_im, $dst_x + $x, $dst_y + $y);
				$OverlayPixel = imagemagic_functions::GetPixelColor($src_im, $x, $y);
				$alphapct = $OverlayPixel['alpha'] / 127;
				$opacipct = $pct / 100;
				$overlaypct = (1 - $alphapct) * $opacipct;

				$newcolor = imagemagic_functions::ImageColorAllocateAlphaSafe(
					$dst_im,
	...
	function gd_info() {
		...
		if (empty($gd_info)) {
			...
			$phpinfo_array = imagemagic_functions::phpinfo_array();
	...
	function filesize_remote($remotefile, $timeout=10) {
		...
		if ($fp = @fsockopen($url['host'], ($url['port'] ? $url['port'] : 80), $errno, $errstr, $timeout)) {
			...
			if (imagemagic_functions::version_compare_replacement(phpversion(), '4.3.0', '>=')) {
	...
	function nonempty_min() {
      ...
      function ApplyMask(&$gdimg_mask, &$gdimg_image) {
        ...
        if (imagemagic_functions::version_compare_replacement(phpversion(), '4.3.2', '>=')) {
            ...
            if ($gdimg_mask_resized = imagemagic_functions::ImageCreateFunction(ImageSX($gdimg_image), ImageSY($gdimg_image))) {
                ...
                if ($gdimg_mask_blendtemp = imagemagic_functions::ImageCreateFunction(ImageSX($gdimg_image), ImageSY($gdimg_image))) {
                    ...
                    for ($x = 0; $x < ImageSX($gdimg_image); $x++) {
                        for ($y = 0; $y < ImageSY($gdimg_image); $y++) {
                            $RealPixel = imagemagic_functions::GetPixelColor($gdimg_image, $x, $y);
                            $MaskPixel = imagemagic_functions::GrayscalePixel(imagemagic_functions::GetPixelColor($gdimg_mask_resized, $x, $y));
                            $MaskAlpha = 127 - (floor($MaskPixel['red'] / 2) * (1 - ($RealPixel['alpha'] / 127)));
                            $newcolor = imagemagic_functions::ImageColorAllocateAlphaSafe($gdimg_mask_blendtemp, $RealPixel['red'], $RealPixel['green'], $RealPixel['blue'], $MaskAlpha);
