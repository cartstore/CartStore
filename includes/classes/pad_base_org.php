    ...
    function _draw_nonstocked_attributes() {
      ...
      foreach($nonstocked_attributes as $nonstocked)
      {
        ...
        switch ($nonstocked['otype']) {
            ...
            $tmp_html = '<input type="text" name ="id[' . TEXT_PREFIX . $nonstocked['oid'] . ']" size="' . $nonstocked['ovals']['products_options_length'] .'" maxlength="' . $nonstocked['ovals']['products_options_length'] . '" value="' . $cart->contents[$_GET['products_id']]['attributes_values'][$nonstocked['ovals']['products_options_id']] .'">  ' . $nonstocked['ovals']['products_options_comment'] ;
            $tmp_html .= $nonstocked['ovals'][0][text] ;

			$out.='<tr><td align="right" class=main><b>'.$nonstocked['oname'].":</b></td><td class=main>".$tmp_html."</td></tr>\n";

            break; // end of text box
            ...
            //CLR 030714 Add logic for radio buttons
              ...
              for ( $cnt=0 ; $cnt < count($nonstocked['ovals']) ; $cnt++) {
				  $tmp_html .= tep_draw_radio_field('id[' . $nonstocked['oid'] . ']', $nonstocked['ovals'][$cnt][id], $checked);
				  $checked = false;
	              $tmp_html .= $nonstocked['ovals'][$cnt][text] ;
		          $tmp_html .=$nonstocked['ovals'][$cnt][commnet] .'<br><br>';

					
			  }
			  /*
			  $tmp_html .= tep_draw_radio_field('id[' . $nonstocked['ovals'][0][id] . ']', $nonstocked['ovals'][0][id], $checked);
              $checked = false;
              $tmp_html .= $nonstocked['ovals'][0][text] ;
              $tmp_html .=$nonstocked['ovals'][0][commnet] ;
