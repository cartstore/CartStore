...
// BOF Price Break for SPPC mod, price break per category
      if ($this->in_cart($products_id)) {
        $this->update_quantity($products_id, $qty, $attributes, $category); //added category
      } else {
        $this->contents[] = array($products_id);
        $this->contents[$products_id] = array('qty' => $qty, 'category' => $category); //added category
// EOF Price Break for SPPC mod, price break per category
...
// BOF Price Break for SPPC mod, price break per category
    function update_quantity($products_id, $quantity = '', $attributes = '', $category = '') { //added category
      global $customer_id;

      if (empty($quantity)) return true; // nothing needs to be updated if theres no quantity, so we return true..

      $this->contents[$products_id] = array('qty' => $quantity, 'category' => $category); //added category
// EOF Price Break for SPPC mod, price break per category
// update database
...
// assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
  ...
  $category_quantity = array(); // calculates no of items per category in shopping basket
  // note that the function adjustQty has not been called yet (invoked by $pf->computePrice), 
  // so this may not be the correct number of items yet
      foreach ($this->contents as $products_id => $contents_array) {
	      if (!isset($category_quantity[$contents_array['category']])) {
		 $category_quantity[$contents_array['category']] = $contents_array['qty'];
	      } else {
		 $category_quantity[$contents_array['category']] += $contents_array['qty'];
	      }
      } // end foreach
...
//   EOF Price Break for SPPC mod, price break per category
      ...
      while (list($products_id, ) = each($this->contents)) {
        $qty = $this->contents[$products_id]['qty'];
	$number_of_items_in_cart_same_category = $category_quantity[$this->contents[$products_id]['category']];
...
//   BOF Price Break for SPPC mod, price break per category
				    'category' => $this->contents[$products_id]['category'],
//   EOF Price Break for SPPC mod, price break per category
