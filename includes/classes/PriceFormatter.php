...
$product_info['specials_new_products_price'] = $specials_price['specials_new_products_price'];
  ...
  function getPriceString($style='productPriceInBox') {
    ...
    /*	$lc_text = '<table align="top" border="1" cellspacing="0" cellpadding="0">';
		...
		. '</div>&nbsp;'
		.'</td></tr>';*/
		$lc_text = '<div class="t8"><div class="special_word">Special Price<br />
 </div><div class="sprice">'
		. $currencies->display_price($this->specialPrice,
				     tep_get_tax_rate($this->taxClass))
		. '<div class="clear"></div></div>
</div><div class="t9">Regular Price:'. $currencies->display_price($this->thePrice,
				     tep_get_tax_rate($this->taxClass)) .'</div>';
        $lc_text .= ' <div class="t10">Save '
	...
	$lc_text .= '<div class="small"> <br />
Or Quantity Pricing: </div>';
		for($i=1; $i<=8; $i++) {
			if($this->quantity[$i] > 0) {
				$lc_text .= '<div class="small">'
				. $this->quantity[$i]
				.'+&nbsp;'
