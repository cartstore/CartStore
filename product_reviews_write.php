...
<!-- body_text //-->
    <table>
      <tr>
    	...
    	<div class="page-header"><h1>Write Review on <?php echo $products_name; ?></h1></div>
  ...
  <?php echo '<a class="btn btn-default btn-sm" href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id', 'action'))) . '">' .  IMAGE_BUTTON_BACK . '</a>'; ?>

  

            <span class="pull-right text-success"><?php echo $products_price; ?></span>
            <hr>
...
<blockquote><?php echo REVIEW_HELP_LINK; ?></blockquote>
<?php
  }
?>
<!-- // Points/Rewards Module V2.00 eof //-->
...
?>
      <p><?php echo $messageStack->output('review'); ?></p>
...
?>
              ...
              <h3><?php echo SUB_TITLE_REVIEW; ?></h3>
             


<?php echo tep_draw_textarea_field('review', 'soft', 40, 15); ?>
<p class="pull-right"><?php echo TEXT_NO_HTML; ?></p>
<br class="clear">
<p class="well lead">

<?php echo '<label class="checkbox-inline"></label><label class="checkbox-inline"> ' . tep_draw_radio_field('rating', '1') . ' 1 Star</label><label class="checkbox-inline">' . tep_draw_radio_field('rating', '2') . ' 2 Stars</label><label class="checkbox-inline"> ' . tep_draw_radio_field('rating', '3') . ' 3 Stars</label><label class="checkbox-inline"> ' . tep_draw_radio_field('rating', '4') . ' 4 Stars</label><label class="checkbox-inline"> ' . tep_draw_radio_field('rating', '5') . ' 5 Stars</label>'; ?>
     </p>         
<p class="lead"> <?php echo '<label>' . SUB_TITLE_FROM . '</label>  <i class="fa fa-user"></i> ' . tep_output_string_protected($customer['customers_firstname'] . ' ' . $customer['customers_lastname']); ?></p>
...
document.write('<?php echo '<p><a class="thumbnail" href="' . tep_href_link(FILENAME_POPUP_IMAGE, 'pID=' . $product_info['products_id']) . '" rel="lightbox">' . tep_image(DIR_WS_IMAGES . $product_info['products_image'], addslashes($product_info['products_name']), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, '') . '<br>' . TEXT_CLICK_TO_ENLARGE . '</a></p>'; ?>');
//--></script>
<noscript>
<?php echo '<p><a class="" href="' . tep_href_link(DIR_WS_IMAGES . $product_info['products_image']) . '" rel="lightbox"">' . tep_image(DIR_WS_IMAGES . $product_info['products_image'], $product_info['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, '') . '<br>' . TEXT_CLICK_TO_ENLARGE . '</a></p>'; ?>
</noscript>
<?php
  }

  echo '<p class="pull-right"><a class="btn btn-default btn-sm" href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now') . '">' . IMAGE_BUTTON_IN_CART . '</a></p>';
?>
             </form>   </td>
      </tr>
    </table>
