<?php
  ...
  $error = false;
  if ((isset($_GET['keywords']) && empty($_GET['keywords'])) && (isset($_GET['dfrom']) && (empty($_GET['dfrom']) || ($_GET['dfrom'] == DOB_FORMAT_STRING))) && (isset($_GET['dto']) && (empty($_GET['dto']) || ($_GET['dto'] == DOB_FORMAT_STRING))) && (isset($_GET['pfrom']) && !is_numeric($_GET['pfrom'])) && (isset($_GET['pto']) && !is_numeric($_GET['pto']))) {
      $error = true;
  ...
  if (empty($dfrom) && empty($dto) && empty($pfrom) && empty($pto) && empty($keywords) && $filter != 1) {
      $error = true;
      $messageStack->add_session('search', ERROR_AT_LEAST_ONE_INPUT);
  }
  if ($error == true) {
...
<div class="page-header"><h1><?php
  ...
  $select_str = "select distinct " . $select_column_list . " m.manufacturers_id,p.map_price, pd.products_short, p.msrp_price, p.products_id,pd.products_url, pd.products_name, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price ";
  ...
  $where_str = " where " . (YMM_FILTER_ADVANCED_SEARCH_RESULT == 'Yes' ? $YMM_where : '') . " p.products_status = '1' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and p2c.categories_id = c.categories_id ";
  ...
  switch ($sort_col) {
      case 'high':
          $order_str .= "products_price desc ";
          break;
      case 'low':
          $order_str .= "products_price asc ";
          break;
      default:
          case 'sortorder':
              $order_str .= "p.pSortOrder ";
              break;
      case 'title':
      default:
          $order_str .= "p.pSortOrder ";
  ...
  echo '<div class="search_wrapper col-lg-12 col-md-12 col-sm-12 col-xs-12">';
  ...
  echo '<a class="btn btn-default" href="' . tep_href_link(FILENAME_ADVANCED_SEARCH, tep_get_all_get_params(array('sort', 'page')), 'NONSSL', true, false) . '">' . IMAGE_BUTTON_BACK . '</a><div class="clear"></div></div></div>';
