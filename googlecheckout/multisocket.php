...
//include_once('googlecheckout/gcxmlparser.php');
//include_once('googlecheckout/gcxmlbuilder.php');
...
//	Change the XML root
	...
	if(isset($xml[0][$root]['calculate']['shipping'])) {
	  $shippers = array();
		$shipping = gc_get_arr_result($xml[0][$root]['calculate']['shipping']['method']);
	...
	$all_results = array();
	$data = array();
	$addresses = gc_get_arr_result($xml[0][$root]['calculate']['addresses']['anonymous-address']);
...
// parallelize shipping requests 
	...
	foreach($results as $result){
		$xmlp = new gcXmlParser(trim($result));
		$root = $xmlp->getRoot();
		$data = $xmlp->getData();  
		$rs = gc_get_arr_result($data[$root]['results']['result']);
		foreach($rs as $r){
		  ...
		  if(isset($r['merchant-code-results'])){
		    $crs = gc_get_arr_result($r['merchant-code-results']['coupon-result']);
		    foreach($crs as $cr){
		  ...
		  if(isset($r['gift-certificate-result'])){
		    $crs = gc_get_arr_result($r['merchant-code-results']['gift-certificate-result']);
		    foreach($crs as $cr){
	...
	foreach($all_results as $id => $results){
	  foreach($results as $result) {
		  $merchant_result = new GoogleResult($id);
			$merchant_result->SetShippingDetails($result, 9999.99, 'USD', 'false');
		  $merchant_calc->AddResult($merchant_result);
	  }
	}
...
function do_xml(&$gcheck, $xml) {
  if(is_array($xml)) {
	  foreach($xml as $key => $value) { 
			...
			if(!gc_is_associative_array($value)){
...
//		To avoid connection Timeout, 0.1 sec window to connect
		switch(socket_select($r = array($sockets[$i]), $write = array($sockets[$i]), $except = array($sockets[$i]), 0 , 100000)){
       case 1:
       				if($debug){
               	echo $i . " [+] Connected\n";
       				}
	...
	while($mid-$start < $timeout){
		...
		for($i=0; $i<count($new_socks); $i++) {
			...
			$rta = strstr("\r\n\r\n", $readed);
			list($respheaders,$responses[]) = paser_http($readed);
			if($debug){
			  print_r($respheaders);
			}
		}
