...
function persistIOPN($NotificationReferenceId, $amazonOrderID, $content) {
        ...
        if(!$isExist && $NotificationReferenceId != null) {
                tep_db_query("insert into " . TABLE_AMAZON_IOPN . " (notificationReferenceID, order_id, notification_txt, created_on)   values ('" . $NotificationReferenceId . "', '" . $amazonOrderID . "', '" . tep_db_input($content) . "', now())");
        }
}

function isDuplicateIOPN($NotificationReferenceId, $amazonOrderID) {
        $iopn_query = tep_db_query("select notificationReferenceID from " . TABLE_AMAZON_IOPN . " where notificationReferenceID = '" . $NotificationReferenceId . "' and order_id = '" . $amazonOrderID . "'");
...
function existIOPN($amazonOrderID) {
        $iopn_query = tep_db_query("select notificationReferenceID from " . TABLE_AMAZON_IOPN . " where order_id = '" . $amazonOrderID . "'");
        if (tep_db_num_rows($iopn_query) > 0)
                return true;
        return false;
}
