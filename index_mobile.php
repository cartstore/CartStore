...
require (DIR_WS_LANGUAGES . $language . '/' . FILENAME_DEFAULT_MOBILE);
...
$category = tep_db_fetch_array($category_query);
		?>
		<?php
		echo '<span class="page_category_listing">
...
<a data-transition="slide" href="' . tep_href_link(FILENAME_DEFAULT_MOBILE, $cPath_new) . '">';


	echo '<div class="imageWrapper">' . tep_image(DIR_WS_IMAGES . $categories['categories_image'], $categories['categories_name'], 82, 82) . '</div>
	
	<h3>' . $categories['categories_name'] . '</h3></a></li>';
	...
	?>
		...
		if (PRODUCT_LIST_FILTER > 1) {
			...
			if (tep_db_num_rows($filterlist_query) > 1) {
				echo '            <td align="center" class="main">' . tep_draw_form('filter', FILENAME_DEFAULT_MOBILE, 'get') . TEXT_SHOW . '';
				if (isset($_GET['manufacturers_id'])) {
		...
		<ul data-divider-theme="a" data-theme="b" data-role="listview" class="ui-listview">
<li class="ui-li ui-li-divider ui-bar-a" role="heading" data-role="list-divider">
				

		<h1 class="product_listing"><?php
		...
		<div class="cat_desc">
			...
			print(tep_image(DIR_WS_IMAGES . $category['categories_image'], $category['categories_name'], '100%', CATEGORY_IMAGE_HEIGHT));
				?></span>
			<?php
